#ifndef ANYBUFFERS_UTIL_H_
#define ANYBUFFERS_UTIL_H_
...
#include <limits.h>
	...
	inline bool CheckDir(const char* file, const char* Dir)
	{
		FILE *fp = NULL;
...
#else
            mkdir(Dir, S_IRWXU | S_IRGRP | S_IXGRP);
            return true;
#endif
    ...
    inline std::string GetFileNameWithoutPostfix(const std::string filename) {
        auto posOfPostfix = filename.find('.', 0);
        if (posOfPostfix == std::string::npos) {
            return filename;
        }

        return filename.substr(0, posOfPostfix);
    }
