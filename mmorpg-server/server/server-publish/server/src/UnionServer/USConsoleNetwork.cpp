...
USConsoleConnection::USConsoleConnection(CLNetwork* network)
:CLNetConnection(network)
{
}

USConsoleConnection::~USConsoleConnection()
{
}

void USConsoleConnection::OnDataReached()
{
	do
	{
...
void USConsoleConnection::OnTick(const Avalon::Time& now)
{
	CLNetConnection::OnTick(now);
}

bool USConsoleConnection::PreHandlePacket(Avalon::Packet* packet)
{
...
void USConsoleConnection::EncodeHeader(UInt8* header, size_t len, UInt16 size, UInt32 id, UInt32 sequence)
{
	*(UInt32*)header = UInt32(size);
	*(UInt16*)(header + sizeof(UInt32)) = 0;
}

void USConsoleConnection::DecodeHeader(UInt8* header, size_t len, UInt16& size, UInt32& id, UInt32& sequence)
{
	size = *(UInt32*)header;
	id = 0xFFFFFFFF;
}
...
UInt32 USConsoleConnection::GenID()
{
	UInt32 ret = 0;
...
USConsoleNetwork::USConsoleNetwork()
{
}
...
Avalon::NetConnection* USConsoleNetwork::CreateConnection()
{
	return new USConsoleConnection(this);
}

bool USConsoleNetwork::Startup(Avalon::PropertySet* properties, CLNetAddress* addresses)
{
...
void USConsoleNetwork::Shutdown(UInt32 inputTimeout)
{
	CLNetwork::Shutdown(inputTimeout);
}


bool USConsoleNetwork::ExecutePacket(Avalon::Packet* packet)
{
	...
	InfoLogStream << "recv request:" << jsonstr << LogStream::eos;
	...
	{
		ErrorLogStream << "USConsoleNetwork::ExecutePacket failed:" << e.what() << LogStream::eos;
		return false;
	}

	USConsoleAgent::Instance()->HandleRequest(nodeid, cmd, subcmd, body);

	return true;
}

void USConsoleNetwork::OnConnected(UInt32 id)
{
	InfoLogStream << "console " << id << " connected!" << LogStream::eos;
}

void USConsoleNetwork::OnDisconnected(UInt32 id)
{
