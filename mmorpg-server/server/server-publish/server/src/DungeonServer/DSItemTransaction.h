#ifndef _DS_ITEM_TRANSACTION_H_
#define _DS_ITEM_TRANSACTION_H_
...
{
	friend class DSPlayer;
	friend class DungeonPlayer;
...
public:
	DSItemTransaction(DSPlayer* player);
	virtual ~DSItemTransaction();

public:
	void SetErrorCode(UInt32 code) { m_ErrorCode = code; }
	UInt32 GetErrorCode() const { return m_ErrorCode; }

	DSPlayer* GetPlayer() const{ return m_pPlayer; }
	...
	void SetDsRaceId(ObjID_t raceId) { m_DsRaceId = raceId; };
	ObjID_t GetDsRaceId() { return m_DsRaceId; }
...
public:
	 ...
	 *@brief 检查完
	 */
	virtual void OnCheckSucced(DSPlayer *pPlayer) = 0;
	virtual void OnCheckFailed(DSPlayer *pPlayer) = 0;

	/**
	*@brief 超时
	*/
	virtual void OnTimeOut(DSPlayer *pPlayer) {}
private:
	void CheckItem();
