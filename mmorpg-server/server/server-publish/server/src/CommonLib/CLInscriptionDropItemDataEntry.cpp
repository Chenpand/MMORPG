/**********************************************************************************

   注意:
           1. _CUSTOM_*_BEGIN和_CUSTOM_*_END之间的代码是可以手动修改的。
           2. 其他地方的代码都不要改动!!!!

*********************************************************************************/
#include "CLInscriptionDropItemDataEntry.h"

// 自定义头文件
// __CUSTOM_HEADER_BEGIN__
// __CUSTOM_HEADER_END__

InscriptionDropItemDataEntry::InscriptionDropItemDataEntry()
{
// __CUSTOM_ENTRYCONSTRUCTOR_BEGIN__
	occuAdditionProb = 0;
// __CUSTOM_ENTRYCONSTRUCTOR_END__
}

InscriptionDropItemDataEntry::~InscriptionDropItemDataEntry()
{
// __CUSTOM_ENTRYDESTRUCTOR_BEGIN__
// __CUSTOM_ENTRYDESTRUCTOR_END__
}

UInt32 InscriptionDropItemDataEntry::GetKey() const
{
// __CUSTOM_GETKEY_BEGIN__
	return id;
// __CUSTOM_GETKEY_END__
}

bool InscriptionDropItemDataEntry::Read(Avalon::DataRow &row)
{
	static UInt32 ID_SEED_XX = 0;
	id = ++ID_SEED_XX;

	groupID = row.ReadUInt32();
	{
		auto str = row.ReadString();
		if (str != "-" && str != "")
		{
			std::vector<std::string> strs;
			Avalon::StringUtil::Split(str, strs, "|");
			for (auto& param : strs)
			{
				auto value = Avalon::StringUtil::ConvertToValue<UInt32>(param);
				chooseNumSet.push_back(value);
			}
		}
	}
	{
		auto str = row.ReadString();
		if (str != "-" && str != "")
		{
			std::vector<std::string> strs;
			Avalon::StringUtil::Split(str, strs, "|");
			for (auto& param : strs)
			{
				auto value = Avalon::StringUtil::ConvertToValue<UInt32>(param);
				numProbSet.push_back(value);
			}
		}
	}
	dataType = row.ReadUInt32();
	itemID = row.ReadUInt32();
	itemProb = row.ReadUInt32();
	itemNum = row.ReadInt32();
	std::string strOccuAddition = row.ReadString();
	if (strOccuAddition != "-")
	{
		std::vector<std::string> strs;
		Avalon::StringUtil::Split(strOccuAddition, strs, "|");
		for (auto& str : strs)
		{
			occuAddition.push_back(Avalon::StringUtil::ConvertToValue<UInt32>(str));
		}
	}

	occuAdditionProb = row.ReadInt32();
// __CUSTOM_READ_BEGIN__
// __CUSTOM_READ_END__
	return true;
}

// __CUSTOM_ENTRYFUNCTION_BEGIN__
// __CUSTOM_ENTRYFUNCTION_END__

bool InscriptionDropItemDataEntryMgr::AddEntry(InscriptionDropItemDataEntry* entry)
{
	if (!Avalon::DataEntryMgrBase<InscriptionDropItemDataEntry>::AddEntry(entry))
	{
		return false;
	}
// __CUSTOM_ADDENTRY_BEGIN__
// __CUSTOM_ADDENTRY_END__
	return true;
}

// __CUSTOM_MGRFUNCTIONDEFINE_BEGIN__
// __CUSTOM_MGRFUNCTIONDEFINE_END__

