...
bool ScoreWar2V2TimeDataEntry::Read(Avalon::DataRow& row)
{
	id = row.ReadUInt32();
	group = row.ReadUInt32();
	status = (ScoreWar2V2Status)row.ReadUInt32();
...
bool ScoreWar2V2TimeDataEntryMgr::AddEntry(ScoreWar2V2TimeDataEntry* entry)
{
	if (!Avalon::DataEntryMgrBase<ScoreWar2V2TimeDataEntry>::AddEntry(entry)) return false;
...
ScoreWar2V2TimeInfo ScoreWar2V2TimeDataEntryMgr::GetScoreWarTimeInfo(Avalon::Time now)
{
	ScoreWar2V2TimeInfo info;
	ScoreWarTimeMap::iterator iter = m_ScoreWarTimeMap.begin();
	Avalon::Date nowData(now);
	while (iter != m_ScoreWarTimeMap.end())
	{
		ScoreWar2V2TimeInfo tmpInfo;
...
bool ScoreWar2V2RewardDataEntry::Read(Avalon::DataRow& row)
{
	id = row.ReadUInt32();
...
bool ScoreWar2V2RewardDataEntryMgr::AddEntry(ScoreWar2V2RewardDataEntry* entry)
{
	if (!Avalon::DataEntryMgrBase<ScoreWar2V2RewardDataEntry>::AddEntry(entry)) return false;
...
ScoreWar2V2RewardDataEntry* ScoreWar2V2RewardDataEntryMgr::GetBaseDataEntry(UInt32 rewardId)
{
	std::map<UInt32, ScoreWar2V2RewardDataEntry*>::iterator iter = m_BaseRewords.find(rewardId);
...
ScoreWar2V2RewardDataEntry* ScoreWar2V2RewardDataEntryMgr::GetRankingDataEntry(UInt32 ranking)
{
	std::vector<ScoreWar2V2RewardDataEntry*>::iterator iter = m_RankingRewards.begin();
