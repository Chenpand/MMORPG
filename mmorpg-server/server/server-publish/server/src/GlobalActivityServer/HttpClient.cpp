...
static std::mutex		s_responseCallBackMutex;
...
static std::vector<std::function<void()>> s_functionsToPerform;
...
{       
    while (true) 
    {
		...
		s_responseCallBackMutex.lock();
		s_functionsToPerform.push_back(std::bind(&HttpClient::dispatchResponseCallbacks, this));
		s_responseCallBackMutex.unlock();
    }
...
public:
    ...
    bool perform(long *responseCode)
    {
        ...
        if (code != CURLE_OK || !(*responseCode >= 200 && *responseCode < 300)) {
			ErrorLogStream << "Curl curl_easy_getinfo failed :" << curl_easy_strerror(code) << LogStream::eos;
            return false;
        }
        // Get some mor data.
...
static void processResponse(HttpResponse* response, char* errorBuffer)
{
    ...
    default:
		ErrorLogStream << "CCHttpClient: unkown request type, only GET and POSt are supported" << LogStream::eos;
        break;
    }
...
void HttpClient::dispatchResponseCallbacks()
{
    ...
    if (response)
    {
        ...
        //void* pTarget = request->getTarget();
...
std::string HttpClient::urlEscape(const char* str, int len)
{
	std::string encodeStr = curl_escape(str, len);
	return encodeStr;
}



std::string HttpClient::urlUnescape(const char* str, int len)
{
	std::string decodeStr = curl_unescape(str, len);
	return decodeStr;
}

// Ö÷Ïß³Ì
void HttpClient::updata()
{
	if( !s_functionsToPerform.empty() ) {
		s_responseCallBackMutex.lock();
		auto temp = s_functionsToPerform;
		s_functionsToPerform.clear();
		s_responseCallBackMutex.unlock();
