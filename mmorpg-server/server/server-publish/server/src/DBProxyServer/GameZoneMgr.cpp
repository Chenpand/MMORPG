...
void GameZoneMgr::OnDisconnected(UInt32 id)
{
	...
	if(zone != NULL)
	{
		...
		class OnDisconnectedVisitor : public GameZoneVisitor
		{
		public:
			OnDisconnectedVisitor(UInt32 zoneId):m_ZoneId(zoneId){}

			bool operator()(GameZone* zone)
			{
				CLProtocol::RecordNotifyOtherDisconnected notify;
				notify.zoneId = m_ZoneId;
		...
		OnDisconnectedVisitor visitor(zoneId);
		VisitGameZones(visitor);
	}
}
