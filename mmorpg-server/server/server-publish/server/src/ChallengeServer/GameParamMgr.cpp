...
bool GameParamMgr::Init()
{
	...
	Avalon::ResultSet* resultSet = dbconn->Query("SELECT * FROM `t_c_gameparam`");
...
void GameParamMgr::SetValue(const char* name, UInt32 value)
{
	...
	if(iter != m_GameParams.end())
	{
		...
		if(dbconn->UpdateFmt("UPDATE `t_c_gameparam` SET `value`=%u WHERE `name`='%s'", iter->second, name) < 0)
		{
	...
	else if(value != 0)
	{
		...
		if(dbconn->UpdateFmt("INSERT INTO `t_c_gameparam`(`name`,`value`) VALUES('%s',%u)", name, value) < 0)
		{
			ErrorLogStream << "execute database statement failed!" << dbconn->GetError() << LogStream::eos;
			CSApplication::Instance()->Exit();
			return;
