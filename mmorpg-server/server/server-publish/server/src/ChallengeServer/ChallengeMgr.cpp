...
{
	Protocol::ChallengeWorldChallengeSigninRet ret;
	ret.id = id;
	ret.result = 0;
...
{
	Protocol::ChallengeWorldChallengeInfoRet ret;
	ret.playerId = id;
	ret.group = 0;
	ret.ranking = 0;
	ret.score = 0;
...
void ChallengeMgr::SendSortList(GameZone* zone, ObjID_t playerId, UInt8 group, UInt16 start, UInt16 num)
{
	...
	Protocol::ChallengeWorldChallengeSortListRet ret;
		...
		iter != sortlist.end(); ++iter)
	{
		...
		{
			Protocol::WorldChallengeSortListEntry entry;
...
{
	Protocol::ChallengeWorldChallengeGetAwardRet ret;
	ret.playerId = playerId;
	ret.type = type;
	ret.index = index;
...
void ChallengeMgr::SetChallengeResult(ObjID_t playerId, UInt8 result, UInt32 score)
{
	...
	if(zone != NULL)
	{
		Protocol::ChallengeWorldChallengeSyncResult sync;
...
void ChallengeMgr::OnTick(const Avalon::Time& now)
{
	UInt32 dayBeginTime = UInt32(GetDayBeginTime(now) / 1000);
	UInt32 seasonTime = GameParamMgr::Instance()->GetValue(SEASON_TIME_VAR);
	if(seasonTime != dayBeginTime - (CURRENT_DATE.WDay() - 1) * DAY_TO_SEC)
	{
		GameParamMgr::Instance()->SetValue(SEASON_TIME_VAR, dayBeginTime - (CURRENT_DATE.WDay() - 1) * DAY_TO_SEC);
		GameParamMgr::Instance()->SetValue(SEASON_END_VAR, 0);
		OnSeasonBegin();
	}

	if(now.Sec() >= m_RoundTime + ROUND_LASTTIME)
	{
		InitRoundTime(now);
		m_RoundStatus = 0;
		m_Pairs.clear();
	...
	if(now.Sec() >= m_RoundTime)
	{
		if(m_RoundStatus == 0)
		{
			if(now.Sec() >= m_RoundTime + ROUND_SIGNIN_TIME)
			{
				m_RoundStatus = 1;
				BeginRound();	//开始新一轮
			}
	...
	if(!m_Pairs.empty())
	{
			...
			iter != m_Scenes.end(); ++iter)
		{
			...
			{
				freeScenes.push_back(scene);
			}
		}

		for(ChallengePairVec::iterator iter = m_Pairs.begin();
			iter != m_Pairs.end();)
		{
			if(freeScenes.empty()) break;

			Challenger* challenger1 = iter->first;
			Challenger* challenger2 = iter->second;

			ChallengeScene* scene = *(freeScenes.begin());
			freeScenes.erase(freeScenes.begin());

			BeginBattle(scene, challenger1, challenger2);

			scene->SetStatus(1);

			iter = m_Pairs.erase(iter);
		}

		if(!m_Pairs.empty() && m_CreateSceneTimer.IsTimeout(now))
		{
...
void ChallengeMgr::BeginBattle(ChallengeScene* scene, Challenger* challenger1, Challenger* challenger2)
{
	...
	packet->SetID(Protocol::SCENE_WORLDCHALLENGE_SEND_CHALLENGERS);
	...
	if(zone1 != NULL)
	{
		Protocol::ChallengeWorldChallengeNotifyBattle notify;
...
void ChallengeMgr::CreateScene()
{
	...
	Protocol::SceneNotifyLoadBattleScene notify;
