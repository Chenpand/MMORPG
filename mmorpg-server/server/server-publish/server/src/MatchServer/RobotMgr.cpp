...
bool RobotMgr::Init()
{
	...
	for (auto data : vistor.allData)
	{
		...
		for (const auto& str : equipStrs)
		{
			...
			raceInfo.wearingEqIds.push_back(equip.uid);
		}
		...
		if (std::find(m_AllOccus.begin(), m_AllOccus.end(), data->occu) == m_AllOccus.end())
		{
			m_AllOccus.push_back(data->occu);
		}
	}
...
std::vector<UInt32> guildBattleOccu = {11, 12, 14};

Robot* RobotMgr::GenRobot(UInt32 mstType, RobotAttrHardType hardType, UInt16 level)
{
	...
	// 公会战机器人特殊处理
	if (mstType == MST_GUILD_BATTLE && level > 50)
	{
		std::random_shuffle(guildBattleOccu.begin(), guildBattleOccu.end());
		std::vector<Robot*> guildBattleRobots;
		for (auto robot : robots)
		{
			if (robot->raceInfo.occu == *(guildBattleOccu.begin()))
			{
				guildBattleRobots.push_back(robot);
			}
		}

		if (guildBattleRobots.empty())
		{
			return NULL;
		}

		return guildBattleRobots[Avalon::Random::RandBetween(0, guildBattleRobots.size() - 1)];
	}

	return robots[Avalon::Random::RandBetween(0, robots.size() - 1)];
}

Robot* RobotMgr::RandOccuRobot()
{
	if (m_AllOccus.empty())
	{
		return NULL;
	...
	UInt8 occu = m_AllOccus[Avalon::Random::RandBetween(0, m_AllOccus.size() - 1)];

	std::vector<Robot*> specRobots;
	for (auto iter = m_Robots.begin(); iter != m_Robots.end(); ++iter)
	{
		for (auto& robot : iter->second)
		{
			if (robot->raceInfo.occu == occu)
			{
				specRobots.push_back(robot);
			}
		}
...
RaceEquip RobotMgr::_GenEquip(UInt32 itemId)
{
	...
	if (itemData->attrId != 0 && itemData->type != ITEM_FASHION)
	{
