...
void MSRouter::OnConnected(UInt32 id)
{
	Avalon::NetConnectionPtr conn = MSInternalNetwork::Instance()->FindConnection(id);
	if (conn == NULL)
	{
...
void MSRouter::OnDisconnected(UInt32 id)
{
...
bool MSRouter::SendToUnion(Avalon::Packet* packet)
{
...
bool MSRouter::SendToUnion(Avalon::Protocol& protocol)
{
	if (m_UnionConnections == NULL)
	{
		ErrorLogStream << "can't find Union connection." << LogStream::eos;
...
bool MSRouter::SendToWorld(UInt32 id, Avalon::Packet* packet)
{
	...
	if (conn == NULL)
	{
		...
		conn = MSInternalNetwork::Instance()->FindConnection(id);
		if (conn == NULL)
		{
			ErrorLogStream << "can't find connection(" << id << ")." << LogStream::eos;
			return false;
...
bool MSRouter::SendToWorld(UInt32 id, Avalon::Protocol& protocol)
{
	Avalon::NetConnectionPtr conn = FindConnection(id);
	if (conn == NULL)
	{
...
bool MSRouter::SendToPlayer(UInt32 wsNodeId, UInt64 playerId, Avalon::Protocol& protocol)
{
...
bool MSRouter::SendToPlayer(UInt32 wsNodeId, UInt64 playerId, Avalon::Packet* packet)
{
...
bool MSRouter::SendToWorldPlayer(UInt32 wsNodeId, UInt64 playerId, Avalon::Protocol& protocol)
{
...
bool MSRouter::SendToWorldPlayer(UInt32 wsNodeId, UInt64 playerId, Avalon::Packet* packet)
{
...
void MSRouter::BroadcastToWorld(Avalon::Protocol& protocol)
{
	Avalon::Packet* packet = Avalon::Packet::Create();
	if (!protocol.Encode(packet))
	{
...
void MSRouter::BroadcastToRelay(Avalon::Protocol& protocol)
{
	Avalon::Packet* packet = Avalon::Packet::Create();
	if (!protocol.Encode(packet))
	{
...
bool MSRouter::SendToRelay(UInt32 nodeid, Avalon::Protocol& protocol)
{
...
Avalon::NetConnectionPtr MSRouter::FindConnection(UInt32 id)
{
	UInt32 servType = CL_GET_SERVERTYPE_FROM_NODEID(id);
	if (servType == ST_WORLD)
	{
