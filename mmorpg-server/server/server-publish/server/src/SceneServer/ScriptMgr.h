...
class ScriptMgr : public CLScriptMgr, public Avalon::Singleton<ScriptMgr>
{
	...
	//EventKey -> ScriptID Set
	using EventKeyToScriptMap = std::map<UInt64, std::set<UInt32>>;

private:
	EventKeyToScriptMap m_EventKeyToScriptMap;
	static std::set<UInt32> m_NullSet;
	
public:
	const std::set<UInt32>& GetScriptIDSet(UInt64 key);
	void RegisterScriptEvent(TaskScriptEventBase& event, Avalon::ScriptFile& script);
