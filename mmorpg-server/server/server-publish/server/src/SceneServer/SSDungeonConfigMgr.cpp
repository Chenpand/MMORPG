...
bool SSActivityDungeonMgr::Init()
{
	...
	class DungeonVisitor : public Avalon::DataEntryVisitor<DungeonDataEntry>
	{
	public:
		bool operator()(DungeonDataEntry* dataEntry)
		{
			...
			if (dataEntry->subType == DST_DEVILDDOM && dataEntry->hardType == LHT_NORMAL)
			{
				std::ostringstream os;
				os << COUNTER_DUNGEON_PROCESS_PREFIX << dataEntry->id;
				CounterCfg::Instance()->RegCounter(os.str().c_str(), COUNTER_CYCLE_DAY, 6);
			}
