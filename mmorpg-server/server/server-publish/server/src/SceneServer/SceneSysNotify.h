...
namespace Sys
{
	...
	*/
	template <typename... Args>
	void SendAnnouncement(UInt64 guildId, UInt32 id, const Args & ... args)
	{
	...
	*/
	template <typename... Args>
	void SendAnnouncementByStartTime(UInt64 guildId, UInt32 id, UInt32 startTime, const Args & ... args)
	{
		AnnouncementDataEntry* dataEntry = AnnouncementDataEntryMgr::Instance()->FindEntry(id);
		if (dataEntry == NULL) return;
