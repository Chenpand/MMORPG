...
void HandleProtocol(Player* player, CLProtocol::SceneTeamMatchStartReq& req)
{
	...
	if (team)
	{
		...
		if (!team->CanStartDungeonRace(req.dungeonId, true, true, teamTaget->type == TEAM_CITY_MONSTER, cityMonster, true))
		{
...
{
	if (protocol.teamId > 0 && protocol.cityMonster.guid > 0 && protocol.cityMonster.type == SOT_CITYMONSTER && protocol.cityMonster.funcType == CITY_MONSTER_ACTIVITY)
	{
