...
bool Team::OnWorldServerStartDungeonRes(UInt32 dungeonId, const SceneNpc& cityMonster)
{
	...
	Protocol::DungenServerTeamStartRaceReq req;
	req.teamId = GetId();
	req.dungeonId = dungeonId;
	...
	{
		Protocol::DungeonFighter fighter;
		fighter.accId = member->GetAccID();
		fighter.roleId = member->GetID();
		fighter.wsId = 0;
		fighter.serviceId = 0;
		fighter.sourceSceneId = CL_MAKE_NODEID(ZONE_ID, SERVER_TYPE, SERVER_ID);
		fighter.pos = seat;
		...
		info.soureSceneId = CL_MAKE_NODEID(ZONE_ID, SERVER_TYPE, SERVER_ID);
		req.playerInfoes.push_back(info);

		++seat;
	}
	Router::SendToDungeon(req);
#else
...
bool Team::OnRelayServerCreateRaceRet(UInt32 relayServerID, UInt32 dungeonId, UInt64 session, Avalon::SockAddr addr, bool isHell, const SceneNpc& cityMonster)
{
	...
	for (auto member : allMembers)
	{
		...
		if (member->CheckOption(SOM_NOT_COUSUME_EBERGY) && dungeonData && dungeonData->threeType == DTT_TEAM_ELITE)
		{
			info.isAssist = info.isAssist | TEAM_MEMBER_MASK_ASSIST;
		}
...
bool Team::CanStartDungeonRace(UInt32 dungeonId, bool checkTicket, bool sendNotice, bool checkLoading, const SceneNpc& cityMonster, bool isTeamMatch)
{
	...
	for (const auto& member : m_Members)
	{
		...
		if (player->CheckOption(SOM_NOT_COUSUME_EBERGY) && DTT_TEAM_ELITE == dungeonData->threeType)
		{
			isAssist = true;
		}

		// 没有次数限制的图没有助战
		if (isAssist && !SSActivityDungeonMgr::Instance()->IsDungeonTimesLimit(dungeonId) && DTT_TEAM_ELITE != dungeonData->threeType)
		{
			isAssist = false;
		}
	...
	if (!hasNormalPlayer)
	{
		if (isTeamMatch && DTT_TEAM_ELITE == dungeonData->threeType)
		{
		}
		else
		{
			// 发送通知，ToDO...
			if (DTT_TEAM_ELITE == dungeonData->threeType)
			{
				master->SendNotify(12017);
			}
			else
			{
...
bool Team::HasFriend(Player * self) const
{
	for (MemberVec::const_iterator iter = m_Members.begin();
		iter != m_Members.end(); ++iter)
	{
		...
		if (player != nullptr)
		{
			if (self->HasRelation(RELATION_FRIEND, iter->roleId))
			{
				return true;
			}
		}
...
bool Team::HasSameGuild(Player * self) const
{
	if (self == nullptr)
	{
		return false;
		...
		iter != m_Members.end(); ++iter)
	{
		...
		if (player != nullptr)
		{
			if (self->GetGuildId() == player->GetGuildId() && self != player)
			{
				return true;
			}
		}
