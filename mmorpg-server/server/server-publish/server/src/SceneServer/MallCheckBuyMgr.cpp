...
MallCheckBuyInfo::MallCheckBuyInfo()
{
	...
	m_Multiple = 0;
	m_MallType = 0;
	m_DeductionCouponId = 0;
	m_TagType = 0;
	m_CreditPointDeduction = 0;
}
...
void MallCheckBuyInfo::Init(const CLProtocol::WorldMallCheckBuy &req)
{
	...
	m_Multiple = req.multiple;
	m_MallType = req.mallType;
	m_DeductionCouponId = req.deductionCouponId;
	m_TagType = req.tagType;
	m_CreditPointDeduction = req.creditPointDeduction;
}
...
void MallCheckBuyInfo::OnBuy()
{
	...
	if (m_MallItemNum == 0 || _IsLimitBuy(counterName))
	{
		return;
	}
	...
	// 抵扣券
	UInt32 removeDeductionCouponNum = 0;
	UInt32 deductionCouponNum = m_Owner->GetItemNum(m_DeductionCouponId);

	bool isNewServerGiftDis = false;

	// 新服礼包折扣
	if (m_MallType == MT_LIMIT_GIFT_PACK
		&& ActivityMgr::Instance()->IsInOpActivityByTmpType(OAT_NEW_SERVER_GIFT_DISCOUNT, GetOwner()->GetLevel())
		&& !m_Owner->GetAccountCounter(ACC_NEW_SERVER_GIFT_DISCOUNT))
	{
		std::vector<OpActivityRegInfo*> OpVec = ActivityMgr::Instance()->GetOpActDataByTmpTypes(OAT_NEW_SERVER_GIFT_DISCOUNT);
		if (!OpVec.empty())
		{
			for (auto it : OpVec.front()->parm2)
			{
				if (it != m_MallItemId)
				{
					continue;
				...
				if (m_MallItemNum == 1)
				{
					cost = m_Cost * (OpVec.front()->parm) / 100;
				}
				else
				{
					UInt32 uintPrice = m_Cost / m_MallItemNum;
					cost = uintPrice * (OpVec.front()->parm) / 100 + (m_MallItemNum - 1) * uintPrice;
				}

				isNewServerGiftDis = true;
			}
		}
	}
	else if (m_TagType == TT_WHOLE_BARGAIN &&
		m_Owner->GetActiveTaskMgr().IsTimeInAcitveType(CURRENT_TIME.Sec(), OAT_WHOLE_BARGAIN_SHOP))
	{
		// 抵扣
		UInt32 deductionVal = 0;
		if (m_DeductionCouponId != 0 && deductionCouponNum > 0)
		{
			ItemDataEntry* entry = ItemDataEntryMgr::Instance()->FindEntry(m_DeductionCouponId);
			if (entry != NULL)
				deductionVal = entry->discountGiftCouponPro;
		}

		// 折扣
		UInt32 discountRate = 0;
		if (m_DiscountCouponId != 0 && discountCouponNum > 0)
		{
			ItemDataEntry* entry = ItemDataEntryMgr::Instance()->FindEntry(m_DiscountCouponId);
			if (entry != NULL)
				discountRate = entry->discountGiftCouponPro;
		}

		// 活动折扣
		UInt32 actDiscount = GameParamMgr::Instance()->GetValue(WHOLE_BARGAIN_DISCOUNT);

		// 按单个算价钱
		cost = 0;
		UInt32 uintPrice = m_Cost / m_MallItemNum;
		UInt32 tempNum = m_MallItemNum;
		while (tempNum--)
		{
			UInt32 finalPrice = uintPrice;

			// 活动折扣
			if (actDiscount > 0)
			{
				finalPrice = (UInt32)(uintPrice * ((float)actDiscount / DISCOUNT_RATE_DENOMINATOR));
			}

			// 消耗多少折扣券
			if (discountCouponNum > 0 && finalPrice > 0)
			{
				finalPrice = (UInt32)(finalPrice * ((float)discountRate / DISCOUNT_RATE_DENOMINATOR));
				++removeDiscountCouponNum;
				--discountCouponNum;
			}

			// 消耗多少抵扣券
			if (deductionCouponNum > 0 && finalPrice > 0)
			{
				finalPrice = (finalPrice > deductionVal ? finalPrice - deductionVal : 0);
				++removeDeductionCouponNum;
				--deductionCouponNum;
			}
	...
	else if (m_DiscountCouponId != 0 && discountCouponNum > 0 && m_MallItemNum > 0)
	{
	...
	ret.m_uRet = _RemoveBuyCost(cost, itemReason);
	...
	if (removeDiscountCouponNum > 0)
	{
		m_Owner->RemoveItem(itemReason.c_str(), m_DiscountCouponId, removeDiscountCouponNum);
	}

	if (removeDeductionCouponNum > 0)
	{
		m_Owner->RemoveItem(itemReason.c_str(), m_DeductionCouponId, removeDeductionCouponNum);
	}
	...
	if (ret.m_uRet == ErrorCode::SUCCESS)
	{
		if ((ItemSubType)m_MoneyType == ST_ITEM_POINT)
		{
			//如果是点券按倍率给商城积分
			UInt32 rate = SystemValueDataEntryMgr::Instance()->GetSystemValue(SVT_TICKET_CONVERT_MALL_INTEGRAL_RATE);
			GetOwner()->AddMallPoint(itemReason.c_str(), cost * m_Multiple * rate);
		}
		...
		// 新服礼包折扣,限账号首次
		if (isNewServerGiftDis)
		{
			m_Owner->SetAccountCounterSyncWorld(ACC_NEW_SERVER_GIFT_DISCOUNT, 1);
		}

		// 购买抵扣券活动
		if (m_MallType == MT_DEDUCTION_COUPON && 
			m_Owner->GetActiveTaskMgr().IsTimeInAcitveType(CURRENT_TIME.Sec(), OAT_WHOLE_BARGAIN_DISCOUNT))
		{
			CLProtocol::GASWholeBargainJoinNumSync sync;
			sync.joinNum = m_MallItemNum;
			m_Owner->SendToGlobalActivity(sync);

			InfoLogStream << PLAYERINFO(m_Owner) << "buy deduction num:" << m_MallItemNum << LogStream::eos;
		}
	}
...
UInt32 MallCheckBuyInfo::_RemoveBuyCost(UInt32 cost, const std::string& reason)
{
	switch (m_MoneyType)
	{
	case ST_BIND_POINT:
		if (m_CreditPointDeduction != 0)
		{
			if (m_Owner->GetPointThree() < cost)
				return ErrorCode::ITEM_NOT_ENOUGH_POINT;
			else
				m_Owner->RemovePointThree(reason.c_str(), cost);
		}
		else
		{
			if (m_Owner->GetPointBoth() < cost)
				...
				m_Owner->RemovePointBoth(reason.c_str(), cost);
		}
		break;
	case ST_ITEM_POINT:
		if (m_CreditPointDeduction != 0)
		{
			if (m_Owner->GetPointCreditPoint() < cost)
				return ErrorCode::ITEM_NOT_ENOUGH_POINT;
			else
				m_Owner->RemovePointCreditPoint(reason.c_str(), cost);
		}
		else
		{
			if (m_Owner->GetPoint() < cost)
				...
				m_Owner->RemovePoint(reason.c_str(), cost);
		}
		break;
	case ST_ITEM_GOLD:
		...
		{
			m_Owner->RemoveGold(reason.c_str(), cost);
		}
		break;
	case ST_BIND_GOLD:
		if (m_Owner->GetBindGold() < cost)
		{
			return ErrorCode::ITEM_NOT_ENOUGH_GOLD;
		}
		else
		{
			m_Owner->RemoveBindGold(reason.c_str(), cost);
		}
		break;
	case ST_MALL_POINT:
		if (m_Owner->GetMallPoint() < cost)
		{
			return ErrorCode::ITEM_DATA_INVALID;
		}
		else
		{
			m_Owner->RemoveMallPoint(reason.c_str(), cost);
		}
		break;
	default:
