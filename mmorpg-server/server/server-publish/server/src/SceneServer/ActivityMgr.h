...
public:
	...
	*/
	...
	bool GetOpenActivityTime(std::string& activeName, UInt32& startTime, UInt32& endTime);
	...
	//通知玩家活动结束
	void NotifyPlayerActiveClosed(ActivityInfo* info);
	...
	*/
	...
	// 加载活动账号数据
	void LoadAccActivityRecord(UInt32 accId);
	void LoadAccActivityRecordResult(UInt32 accId, ActivityAccRecord* record);
	...
	// 删除过期的账号活动记录
	void DelAccActivityRecord(UInt32 accid);

	// 数据库删除账号活动记录
	void DBDelAccActivityRecord(UInt32 accid);
	void DelAccActivityRecord(UInt64 guid);

private:
	bool _AddOpActData(OpActivityRegInfo& info);
