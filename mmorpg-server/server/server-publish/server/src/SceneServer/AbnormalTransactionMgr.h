...
public:
	...
	*@brief tick函数
	*/
	static void OnTick(const Avalon::Time& now);

	static void Init();

	static void QueryWaitFreezeAbnormalTransNew();

	static void OnQueryWaitFreezeAbnormalTransactionRecordRetNew(CLRecordCallback* callback);

	void HandleOneRecords(AbnormalTransactionRecordVec& waitRecords);
...
private:
	...
	// 检测拍卖行超时物品定时器
	static Avalon::SimpleTimer	 m_checkTimer;
};
