...
Avalon::SimpleTimer	AbnormalTransactionMgr::m_checkTimer;

class QueryWaitFreezeAbnormalTransactionCallbackNew : public CLRecordCallback
{
public:
	QueryWaitFreezeAbnormalTransactionCallbackNew() {}

	virtual void OnFinished()
	{
		AbnormalTransactionMgr::OnQueryWaitFreezeAbnormalTransactionRecordRetNew(this);
	}

	virtual void OnFailed(UInt32 errorcode)
	{
		ErrorLogStream << "select t_abnormal_transaction failed!" << LogStream::eos;
	}
...
void AbnormalTransactionMgr::OnHeartbeat(const Avalon::Time& now, int tickType)
{
	...
	else if (tickType == LOGICTICK_FIVEMIN)
	{
		if (!SSApplication::Instance()->IsUseFreezeAbnormalNew())
		{
			QueryWaitFreezeAbnormalTrans();
		}
	}
...
void AbnormalTransactionMgr::OnTick(const Avalon::Time& now)
{
	if (m_checkTimer.IsTimeout(now))
	{
		if (SSApplication::Instance()->IsUseFreezeAbnormalNew())
		{
			QueryWaitFreezeAbnormalTransNew();
		}
	}
}

void AbnormalTransactionMgr::Init()
{
	m_checkTimer.SetInterval(5 * 60 * 1000);
}

void AbnormalTransactionMgr::QueryWaitFreezeAbnormalTransNew()
{
	...
	//selectCmd->PutCond("seller", GetOwner()->GetID());
	//selectCmd->PutCond("freeze_object", (UInt8)AT_FREEZE_OBJECT_SELLER);
	std::string limitStr = "ORDER BY guid limit 100";
	selectCmd->SetLimit(limitStr);
	CLRecordClient::Instance()->SendCommand(selectCmd, new QueryWaitFreezeAbnormalTransactionCallbackNew());
}

void AbnormalTransactionMgr::OnQueryWaitFreezeAbnormalTransactionRecordRetNew(CLRecordCallback* callback)
{
	if (!callback) return;
	if (SSApplication::Instance()->IsCloseAbnormalTransHandle()) return;

	if (!callback->NextRow()) return;

	std::map<ObjID_t, std::vector<AbnormalTransactionRecord*>> records;

	do{
		UInt8 freeze_object = callback->GetData("freeze_object");
		ObjID_t buyer = callback->GetData("buyer");
		ObjID_t seller = callback->GetData("seller");

		if (freeze_object == AT_FREEZE_OBJECT_SELLER)
		{
			Player* player = PlayerMgr::Instance()->FindNormalPlayer(seller);
			if (!player)
			{
				continue;
			}
			...
			records[seller].push_back(record);
		}
		else if (freeze_object == AT_FREEZE_OBJECT_BUYER)
		{
			Player* player = PlayerMgr::Instance()->FindNormalPlayer(buyer);
			if (!player)
			{
				continue;
			}
			...
			records[buyer].push_back(record);
		}
	} while (callback->NextRow());

	for (auto iter : records)
	{
		Player* player = PlayerMgr::Instance()->FindNormalPlayer(iter.first);
		if (!player)
		{
			continue;
		}
		player->GetAbnormalTransactionMgr().HandleOneRecords(iter.second);
...
void AbnormalTransactionMgr::HandleOneRecords(AbnormalTransactionRecordVec& waitRecords)
{
	if (waitRecords.empty())
	{
		ErrorLogStream << PLAYERINFO(GetOwner()) << "Wait records are empty!" << LogStream::eos;
	...
	AbnormalTransactionRecord* record = NULL;
	for (size_t i = 0; i < waitRecords.size(); ++i)
	{
		if (FindAbnormalRecord(waitRecords[i]->GetID()) == NULL)
		{
			record = waitRecords[i];
			waitRecords.erase(waitRecords.begin() + i);
			break;
		}
	}
