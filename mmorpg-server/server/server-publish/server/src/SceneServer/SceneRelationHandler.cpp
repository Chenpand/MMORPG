...
{
	Player* player = PlayerMgr::Instance()->FindPlayer(req.id);
	if (player == NULL)
	{
		ErrorLogStream << "SceneRelationSyncList not find player id = " << req.id << LogStream::eos;
		return;
	}
...
class PlayerLoadingForFreindGiveTask : public PlayerLoadingCallback
{
public:
	PlayerLoadingForFreindGiveTask(UInt64 friendId, UInt16 intimacy, UInt16 incIntimacy, UInt8 isMaster)
		: m_friendId(friendId), m_intimacy(intimacy), m_incIntimacy(incIntimacy), m_isMaster(isMaster){}
	virtual void OnFinish(Player* player)
	{
		DebugLogStream << "PlayerLoadingForFreindGiveTask, playerId(" << m_friendId << "), m_intimacy : " << m_intimacy
			<< "" << LogStream::eos;
		...
		if (m_isMaster && !player->HasRelation(RELATION_MASTER, m_friendId))
		{
			player->AddRelation(RELATION_MASTER, m_friendId);
		}

		player->OnSyncIntimacy(m_friendId, m_intimacy, m_incIntimacy);
...
void HandleProtocol(CLProtocol::SceneRelationSyncIntimacy& req)
{
	...
	DebugLogStream << "HandleProtocol SceneRelationSyncIntimacy, playerId(" << req.id << "), friendId : " << req.friendId << LogStream::eos;

	if (player)
	{
		if (req.isMaster && !player->HasRelation(RELATION_MASTER, req.friendId))
		{
			player->AddRelation(RELATION_MASTER, req.friendId);
		}
	...
	if (player == NULL && req.incIntimacy > 0)
	{
		DebugLogStream << "HandleProtocol SceneRelationSyncIntimacy player == NULL, playerId(" << req.id << "), start off load" << LogStream::eos;
		PlayerMgr::Instance()->LoadOfflinePlayerCb(req.id, 0, new PlayerLoadingForFreindGiveTask(req.friendId, req.intimacy, req.incIntimacy, req.isMaster));
	}
}
...
public:
	...
	virtual void OnFinish(Player* player)
	{
		...
		m_Ret.success = 1;
		player->OnUpdateTotalEquipScore();
		player->GetPetMgr().CalcAllPetsSocre();
...
void HandleProtocol(Player * player, CLProtocol::SceneQueryHireRedPointReq & req)
{
	player->SetCounter(COUNTER_HIRE_RED_POINT, 1);
}
