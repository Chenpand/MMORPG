...
{
	// 荣誉带来的折扣
	ShopDataEntry* shopData = ShopDataEntryMgr::Instance()->FindEntry(m_pDataEntry->shopId);
	if (shopData != NULL)
	{
		if (shopData->isHonorDiscount != 0)
		{
			Player* player = PlayerMgr::Instance()->FindPlayer(m_owner);
			if (player != NULL)
			{
				m_DiscountRate = HonorLevelDataEntryMgr::Instance()->GetShopDiscount(player->GetHonorLevel(), m_pDataEntry->shopId);
			}
