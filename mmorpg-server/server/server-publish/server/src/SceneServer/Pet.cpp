...
void Pet::IncExp(UInt32 exp)
{
	...
	DebugLogStream << PLAYERINFO(GetOwner()) << "Pet(" << GetID() << ") IncExp(" << exp << ") Level(" << oldLevel << "->" << m_Level << ") Exp(" << oldExp << "->" << m_Exp << ")" << LogStream::eos;
}
...
void Pet::ConsumeHunger(UInt16 value)
{
	...
	DebugLogStream << PLAYERINFO(GetOwner()) << "Pet(" << GetID() << ") ConsumeHunger(" << value << ") Hunger(" << oldHunger << "->" << m_Hunger << ")" << LogStream::eos;
}
...
void Pet::AddHunger(UInt16 value)
{
	...
	DebugLogStream << PLAYERINFO(GetOwner()) << "Pet(" << GetID() << ") AddHunger(" << value << ") Hunger(" << oldHunger << "->" << m_Hunger << ")" << LogStream::eos;
}
