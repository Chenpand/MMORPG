...
public:
	...
	//开罐子
	UInt32	OpenJar(UInt32 id, UInt8 combo, UInt32& getPointId, UInt32& getPoint, UInt32& getPointCrit, 
	...
	//开罐子规则的礼包
	UInt32 OpenJarGift(UInt32 jarid, std::string giftItemName, RewardGroup& rewardGroup, bool bNotify = false, std::string reason = "");

	//开罐子规则的礼包
	//UInt32 OpenJarGift(UInt32 jarid, std::string giftItemName, RewardGroup& rewardGroup);

	//处理打开罐子礼包
	void OnOpenJarGift(UInt32 jarid, std::string giftItemName);
...
private:
	...
	JarItem* HitProc(BonusVec& pool, UInt32 smellType = 0);
