...
void HandleProtocol(Player* player, CLProtocol::SceneMatchStartReq &req)
{
	...
	*/
	...
	else if (msReq.type == PK_2V2_ACTIVITY || msReq.type == PK_5V5_MATCH)
	{
		//CLProtocol::MatchServerPkRoomReq req;
		//req.pkType = (UInt8)PK_2V2_ACTIVITY;
		//req.roomId = 0;
		//PkRoomMatchPlayerInfo info;
		//info.wsId = CL_MAKE_NODEID(ZONE_ID, ST_WORLD, 1);
		//info.accid = player->GetAccID();
		...
		//info.winStreak = player->GetSeasonPkWinStraight();
		//info.name = player->GetName();
		//info.score = player->Get2V2MatchScore();
		//info.seasonLevel = player->GetSeasonLevel();
		//info.seasonStar = player->GetSeasonStar();
		//info.seasonExp = player->GetSeasonExp();
		//info.seat = 0;
		...
		//Router::SendToWorld(req);
		//return;
		msReq.matchScore = player->Get2V2ScoreWarScore();
	}
	else if (msReq.type == PK_EQUAL_1V1)
	{
		//分数暂置为0，随机匹配
		msReq.matchScore = 0;
		msReq.level = PlayerDataEntryMgr::Instance()->GetMaxLevel();
	}
    else
    {
...
void HandleProtocol(CLProtocol::MatchServerQueryPlayerInfoReq& req)
{
    ...
    else
    {
		...
		{
			res.racePlayerInfo = player->GetMatchRacePlayerInfo(true, static_cast<MatchQueryType>(req.type));
		}
		else if (req.type == MATCH_QUERY_EQUAL_RACE_INFO)
		{
			res.racePlayerInfo = player->GetEqualMatchRacePlayerInfo();
		}
		else
		{
...
void HandleProtocol(CLProtocol::UnionMatchServerQueryPlayerInfoReq& req)
{
	...
	res.msg.sessionId = req.msg.sessionId;

	Player* player = PlayerMgr::Instance()->FindPlayer(req.msg.roleId);
	if (!player)
	{
	...
	{
		res.msg.roleId = player->GetID();
		res.msg.result = MATCH_QUERY_SUCCESS;
		res.msg.type = req.msg.type;
		if (req.msg.type == MATCH_QUERY_RACE_INFO || req.msg.type == MATCH_QUERY_GUILD_RACE_INFO || req.msg.type == MATCH_PREMIUM_LEAGUE_PRELIMINAY_INFO)
		{
			res.msg.racePlayerInfo = player->GetMatchRacePlayerInfo(true, static_cast<MatchQueryType>(req.msg.type));
		}
		else if (req.msg.type == MATCH_QUERY_EQUAL_RACE_INFO)
		{
			res.msg.racePlayerInfo = player->GetEqualMatchRacePlayerInfo();
		}
		else
		{
			res.msg.dungeonPlayerInfo = player->GetDungeonMgr().GetDungeonPlayerInfo(req.msg.dungeonId);
		}
	}
