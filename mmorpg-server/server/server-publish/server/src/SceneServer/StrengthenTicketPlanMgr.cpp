...
UInt32 StrengthenTicketPlanMgr::SynthesisStrengthenTicket(Player* player, UInt32 planId)
{
	...
	OpActTask* reduceCostMatTask = NULL;
	// 祈福活动
	player->GetActiveTaskMgr().VisitOpenOpActAndTaskByOpType(OAT_DAILY_BUFF, [&](const OpActivityRegInfo* opAct, const OpActTaskData& taskData, OpActTask* task){
		...
		{
			ErrorLogStream << opAct->LogStr() << " task(" << task->dataId << ")'s data variable size is error!" << LogStream::eos;
			return true;
		}

		UInt32 type = taskData.variables.front();

		if (type != PrayActivityBuffType::PABT_SYNTH_STREN_TICKET_REDUCE_MAT)
		{
			return true;
		}
		...
		{
			ErrorLogStream << opAct->LogStr() << " task(" << task->dataId << ")'s data variable2 size is error!" << LogStream::eos;
			return true;
		}

		UInt32 count = taskData.variables2.front();

		if (task->GetParamVar(OPACT_TIMES_VAR_KEY) < count)
		{
			reduceCostMatRates = taskData.rewards;
			reduceCostMatTask = task;
		}
	...
	// 只有非绑定的才计数
	if (reduceCostMatTask && (IBTP_NOT == synthesisData->binding))
	{
		UInt32 reduceCostMatTimes = reduceCostMatTask->GetParamVar(OPACT_TIMES_VAR_KEY) + 1;
		reduceCostMatTask->SetParamVar(OPACT_TIMES_VAR_KEY, reduceCostMatTimes);

		player->GetActiveTaskMgr().OnUpdateOpTaskParamVar(reduceCostMatTask);
	}
