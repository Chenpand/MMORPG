...
void ActivityMgr::OnActOpen(ActivityInfo* info)
{
	if (!info) return;

	class ActOpenVisit : public PlayerVisitor
	{
	public:
		ActOpenVisit(ActivityInfo* info) : m_Info(info) {}

		bool operator()(Player* player)
		{
			player->OnActiveOpen(m_Info);
			return true;
		}
...
void ActivityMgr::OpenTribeActivity(ObjID_t uTribeId, const std::string& name, UInt16 level, UInt32 dueTime)
{
	...
	ActivityVec::iterator iter = activities.begin();
...
bool ActivityMgr::IsInActivity(const std::string& name) const
{
	...
	{
		if (iter->name == name && 1 == iter->state)
		{
			return true;
		}
	}
...
bool ActivityMgr::IsInTribeActivity(ObjID_t uTribeId, const std::string& name) const
{
	...
	if (itertribe == m_TribeActivities.end())
	{
		return false;
	}

	const ActivityVec& activities = itertribe->second;
	ActivityVec::const_iterator iter = activities.begin();
	for (; iter != activities.end(); ++iter)
	{
		if (iter->name == name)
		{
...
void ActivityMgr::GetOpenActivity(Player* player, std::vector<ActivityInfo>& activities)
{
	...
	for (auto activity : m_Activities)
	{
		...
		if (activity.state != 0 && activity.level <= player->GetLevel())
		{
...
void ActivityMgr::NotifyPlayerAcceptActiveTask(UInt32 id)
{
	...
	{
	public:
		AcceptActiveTaskVisit(UInt32 activeId) : m_activeId(activeId) {}
...
void ActivityMgr::NotifyPlayerRemoveActiveTask(UInt32 id)
{
	...
	{
	public:
		RemoveActiveTaskVisit(UInt32 activeId) : m_activeId(activeId) {}
...
void ActivityMgr::NotifyPlayerActiveClosed(ActivityInfo* info)
{
	if (!info) return;
	...
	{
	public:
		NotifyPlayerActiveClosedVisit(ActivityInfo* info_) : m_info(info_) {}
		
		bool operator()(Player* player)
		{
			player->OnActiveClosed(m_info);
			return true;
		}
	...
	NotifyPlayerActiveClosedVisit visitor(info);
	PlayerMgr::Instance()->VisitNormalPlayers(visitor);
}
...
void ActivityMgr::OnLoadOpActData(OpActRegMap& activeData)
{
	...
	public:
		...
		bool operator()(Player* player)
		{
			...
			for (auto& info : m_protocol.datas)
			{
				...
				if (!player->GetActiveTaskMgr().IsMeetOpActRequireByPlayer(&info))
				{
					info.state = AS_END;
				}
				...
				if (info.state == AS_IN)
				{
					player->GetActiveTaskMgr().OnOpActOpen(&info);
				}
			}
	...
	{
		if (_AddOpActData(itr->second))
		{
			opActProtocol.datas.push_back(itr->second);
		}

		// 全名砍价团购
		if (itr->second.tmpType == OAT_WHOLE_BARGAIN_SHOP && itr->second.state == AS_IN)
		{
			CLProtocol::GASWholeBargainDiscountQuery req;
			req.actId = itr->second.parm;
...
void ActivityMgr::OnOpActStateChange(OpActivityRegInfo& info)
{
	...
	InfoLogStream << "On OpActivity change...(id=" << info.dataId << ", state=" << info.state
...
void ActivityMgr::OnOpActOpen(OpActivityRegInfo* info)
{
	...
	InfoLogStream << "OpActivity(id=" << info->dataId << ", state=" << info->state
		<< ", preT=" << TimeUtil::TimestampToStandardFormat(info->prepareTime)
		<< ", st=" << TimeUtil::TimestampToStandardFormat(info->startTime)
		<< ", et=" << TimeUtil::TimestampToStandardFormat(info->endTime)
		<< ") open." << LogStream::eos;
	...
	public:
		...
		bool operator()(Player* player)
		{
			player->GetActiveTaskMgr().OnOpActOpen(m_Info);
			return true;
		}
...
void ActivityMgr::OnOpActClosed(OpActivityRegInfo* info)
{
	...
	public:
		...
		bool operator()(Player* player)
		{
			player->GetActiveTaskMgr().OnOpActClosed(m_Info);
			return true;
		}
...
void ActivityMgr::NotifyPlayerOpActChange(OpActivityRegInfo& info)
{
	...
	public:
		...
		bool operator()(Player* player)
		{
			if (!player->GetActiveTaskMgr().IsMeetOpActRequireByPlayer(&m_protocol.info))
			{
				return true;
			}
...
bool ActivityMgr::IsInOpActivity(const OpActivityRegInfo* opAct, Player* player)
{
	...
	if (!player->GetActiveTaskMgr().IsMeetOpActRequireByPlayer(opAct))
	{
		return false;
	}
...
UInt32 ActivityMgr::GetOpActivityAddition(UInt32 tmpId, Player* player)
{
	...
	//DebugLogStream << "Get OpActivity(" << tmpId << ") addition(" << addition << ")." << LogStream::eos;
	return addition;
}
...
bool ActivityMgr::GetOpenActivityTime(std::string& activeName, UInt32& startTime, UInt32& endTime)
{
	for (const auto& activity : m_Activities)
	{
		if (activity.name == activeName && activity.state == AS_IN)
		{
			startTime = activity.startTime;
			endTime = activity.dueTime;
			return true;
...
bool ActivityMgr::_AddOpActData(OpActivityRegInfo& info)
{
	OpActivityRegInfo* data = NULL;

	InfoLogStream << "Add OpAct(" << info.dataId << ", " << info.name << "), state(" << info.state << ") sync..." << LogStream::eos;
...
*@brief 查询运营活动账号记录
*/
class SelectOpActivityAccCallback : public CLRecordCallback
{
public:
	SelectOpActivityAccCallback(UInt32 accId) : m_AccId(accId) {}

	void OnFinished()
	{
		while (NextRow())
		{
			ActivityAccRecord* accRecord = new ActivityAccRecord();
			accRecord->guid = GetKey();
			accRecord->actId = GetData("activity_id");
			accRecord->curNum = GetData("cur_num");
			accRecord->state = GetData("states");
			ActivityMgr::Instance()->LoadAccActivityRecordResult(m_AccId, accRecord);
		}
	}

	void OnFailed()
	{
		ErrorLogStream << " select t_activity_account_record failed! accId:" << m_AccId << LogStream::eos;
	}
private:
...
void ActivityMgr::LoadAccActivityRecord(UInt32 accId)
{
	//请求活动账号数据
	CLSelectCommand* actAcc = CLRecordClient::Instance()->PrepareSelectCommand("t_activity_account_record");
	actAcc->PutCond("acc_id", accId);
	CLRecordClient::Instance()->SendCommand(actAcc, new SelectOpActivityAccCallback(accId));
}

void ActivityMgr::LoadAccActivityRecordResult(UInt32 accId, ActivityAccRecord* record)
{
	if (!record) return;

	ActivityAccRecord* curRecord = FindAccActivityRecord(accId, record->actId);
	if (NULL == curRecord)
	{
	...
	// 合服处理
	if (record->curNum > curRecord->curNum)
	{
		curRecord->guid = record->guid;
		curRecord->curNum = record->curNum;
		curRecord->state = record->state;
		UpdateAccActivityRecord(accId, curRecord);
	}

	DelAccActivityRecord(record->guid);
...
void ActivityMgr::UpdateAccActivityRecord(UInt32 accid, const ActivityAccRecord* record, bool isInsert)
{
	...
	if (isInsert)
	{
		CLInsertCommand* insert = CLRecordClient::Instance()->PrepareInsertCommand("t_activity_account_record", record->guid);
	...
	{
		CLUpdateCommand* update = CLRecordClient::Instance()->PrepareUpdateCommand("t_activity_account_record", record->guid, true);
...
void ActivityMgr::DelAccActivityRecord(UInt32 accid)
{
	auto iter = m_accActivityRecordMap.find(accid);
	if (iter == m_accActivityRecordMap.end())
		return;
...
void ActivityMgr::DBDelAccActivityRecord(UInt32 accid)
{
...
void ActivityMgr::DelAccActivityRecord(UInt64 guid)
{
	CLDeleteCommand* del = CLRecordClient::Instance()->PrepareDeleteCommand("t_activity_account_record", guid);
	CLRecordClient::Instance()->SendCommand(del);
}