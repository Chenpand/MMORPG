...
public:
	void OnFinished(Player* player)
	{
		while (NextRow())
		{
			player->GetActiveTaskMgr().OnLoadOpActTasks(GetData("task_info_str").GetString());
		}
	}
...
public:
	void OnFinished(Player* player)
	{
		player->GetActiveTaskMgr().OnLoadNewOpActTasks(this);
	}
...
*@brief 查找运营活动账号任务回调
*/
class SelectOpAccountTaskCallback : public SelectRoleInfoCallback
{
	AVALON_DEFINE_OBJECTPOOL(SelectOpAccountTaskCallback, Avalon::Mutex)
public:
	SelectOpAccountTaskCallback(Player* player)
		:SelectRoleInfoCallback(player) {}
	~SelectOpAccountTaskCallback() {}

public:
	void OnFinished(Player* player)
	{
		player->GetActiveTaskMgr().OnLoadOpActAccountTasks(this);
	}

	void OnFailed(Player* player)
	{
		GameErrorLogStream("Player") << "player(" << player->GetAccID() << "," << player->GetID()
			<< "," << player->GetName() << ") select operate account task failed." << LogStream::eos;
	}
};
...
public:
    void OnFinished(Player* player)
    {
        while (NextRow())
        {
            ...
            TaskStatus status = (TaskStatus)(UInt32)GetData("status");
            std::string parameter = GetData("parameter").GetString();
            UInt32 acceptTime = GetData("accepttime");

            player->GetActiveTaskMgr().OnLoadTask(guid, dataId, status, parameter, acceptTime);
        }
    }
    ...
    {
        GameErrorLogStream("Player") << "player(" << player->GetAccID() << "," << player->GetID()
            << "," << player->GetName() << ") select activetask failed." << LogStream::eos;
    }
...
public:
	void OnFinish(Player* player, const CLProtocol::WorldQueryAccountRet& res)
    {
		...
		player->InitCreditPoint(res.creditPoint);
		...
		player->InitAccountAchievementScore(res.accountAchievementScore);
		player->InitMallPoint(res.mallPoint);
		player->InitAdventureCoin(res.adventureCoin);
		...
		DebugLogStream << "player(" << player->GetID() << ", " << player->GetAccID() << ") vip(" << player->GetVipLvl() << ", "
			<< player->GetVipExp() << ") point(" << player->GetPoint() << ") creditPoint(" << player->GetCreditPoint() << ") totalCharge(" 
			<< res.totalChargeNum << ") storageSize(" << res.storageSize << ")." << LogStream::eos;
    }
...
*@brief 查询头像框
*/
class SelectWeekSignCallback : public SelectRoleInfoCallback
{
	AVALON_DEFINE_OBJECTPOOL(SelectWeekSignCallback, Avalon::Mutex)
public:
	SelectWeekSignCallback(Player* player)
		:SelectRoleInfoCallback(player) {}
	~SelectWeekSignCallback() {}

public:
	virtual void OnFinished(Player* player)
	{
		if (!player) return;
		player->GetActiveTaskMgr().LoadWeekSign(this);
	}

	virtual void OnFailed(Player* player)
	{
		if (!player) return;
		ErrorLogStream << PLAYERINFO(player) << " select t_week_sign failed!" << LogStream::eos;
	}
};
...
*@brief 查询物品暂存
*/
class SelectItemDepositCallback : public SelectRoleInfoCallback
{
	AVALON_DEFINE_OBJECTPOOL(SelectItemDepositCallback, Avalon::Mutex)
public:
	SelectItemDepositCallback(Player* player)
		:SelectRoleInfoCallback(player) {}
	~SelectItemDepositCallback() {}

public:
	virtual void OnFinished(Player* player)
	{
		if (!player) return;
		player->GetItemDepositMgr().LoadItemDeposit(this);
	}

	virtual void OnFailed(Player* player)
	{
		if (!player) return;
		ErrorLogStream << PLAYERINFO(player) << " select t_item_deposit failed!" << LogStream::eos;
	}
};
...
*@brief 查询团本
*/
class SelectTeamCopyCallback : public SelectRoleInfoCallback
{
	AVALON_DEFINE_OBJECTPOOL(SelectTeamCopyCallback, Avalon::Mutex)
public:
	SelectTeamCopyCallback(Player* player)
		:SelectRoleInfoCallback(player) {}
	~SelectTeamCopyCallback() {}

public:
	virtual void OnFinished(Player* player)
	{
		if (!player) return;
		player->GetTeamCopyMgr().LoadData(this);
	}

	virtual void OnFailed(Player* player)
	{
		if (!player) return;
		ErrorLogStream << PLAYERINFO(player) << " select t_team_copy failed!" << LogStream::eos;
	}
};
...
*@brief 查询道具回调
*/
class SelectEquipSchemeCallback : public SelectRoleInfoCallback
{
	AVALON_DEFINE_OBJECTPOOL(SelectEquipSchemeCallback, Avalon::Mutex)
public:
	SelectEquipSchemeCallback(Player* player)
		:SelectRoleInfoCallback(player) {}
	~SelectEquipSchemeCallback() {}

public:
	void OnFinished(Player* player)
	{
		if (!player) return;
		player->GetEqSchemeMgr().LoadDataFromDb(this);
	}

	void OnFailed(Player* player)
	{
		GameErrorLogStream("Player") << "player(" << player->GetAccID() << "," << player->GetID()
			<< "," << player->GetName() << ") select equip scheme failed." << LogStream::eos;
	}
};
...
*@brief 查询历史荣誉
*/
class SelectHistoryHonorCallback : public SelectRoleInfoCallback
{
	AVALON_DEFINE_OBJECTPOOL(SelectHistoryHonorCallback, Avalon::Mutex)
public:
	SelectHistoryHonorCallback(Player* player)
		:SelectRoleInfoCallback(player) {}
	~SelectHistoryHonorCallback() {}

public:
	virtual void OnFinished(Player* player)
	{
		if (!player) return;
		player->GetHistoryHonor().LoadData(this);
	}

	virtual void OnFailed(Player* player)
	{
		if (!player) return;
		ErrorLogStream << PLAYERINFO(player) << " select t_honor_history failed!" << LogStream::eos;
	}
};

//查询快捷键
class SelectShortcutKeyCallback : public SelectRoleInfoCallback
{
	AVALON_DEFINE_OBJECTPOOL(SelectShortcutKeyCallback, Avalon::Mutex)
public:
	SelectShortcutKeyCallback(Player* player) : SelectRoleInfoCallback(player) {}
	~SelectShortcutKeyCallback() {}
...
public:
	void OnFinished(Player* player)
	{
		while (NextRow())
		{
			UInt32 type = GetData("type");
			std::string value = GetData("value").GetString();

			player->OnLoadShortcutKeyData(type, value);
		}
	}
	...
	{
		GameErrorLogStream("Player") << "player(" << player->GetAccID() << "," << player->GetID()
			<< "," << player->GetName() << ") select shortcut key failed." << LogStream::eos;
	}
...
*@brief 查询地牢数据回调
*/
class SelectLostDungeonCallback : public SelectRoleInfoCallback
{
	AVALON_DEFINE_OBJECTPOOL(SelectLostDungeonCallback, Avalon::Mutex)
public:
	SelectLostDungeonCallback(Player* player)
		:SelectRoleInfoCallback(player) {}
	~SelectLostDungeonCallback() {}

public:
	void OnFinished(Player* player)
	{
		if (!player) return;
		player->GetLostDungeon().LoadDataFromDb(this);
	}

	void OnFailed(Player* player)
	{
		GameErrorLogStream("Player") << "player(" << player->GetAccID() << "," << player->GetID()
			<< "," << player->GetName() << ") select lost dungeon failed." << LogStream::eos;
	}
};
...
*@brief 查询其他角色数据回调
*/
class SelectOtherRoleCallback : public SelectRoleInfoCallback
{
	AVALON_DEFINE_OBJECTPOOL(SelectOtherRoleCallback, Avalon::Mutex)
public:
	SelectOtherRoleCallback(Player* player)
		:SelectRoleInfoCallback(player) {}
	~SelectOtherRoleCallback() {}

public:
	void OnFinished(Player* player)
	{
		if (!player) return;
		player->LoadOtherRoleData(this);
	}

	void OnFailed(Player* player)
	{
		GameErrorLogStream("Player") << "player(" << player->GetAccID() << "," << player->GetID()
			<< "," << player->GetName() << ") select all role failed." << LogStream::eos;
	}
};
