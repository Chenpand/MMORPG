...
void Player::ChangeBattleScene(UInt32 curDoorId, UInt32 dstSceneId, UInt32 dstDoorId, UInt8 battleType)
{
	...
	try
	{
		// 当请求进入吃鸡场景的时候，curDoorId代表battleId, dstDoorId代表节点ID
...
void Player::OnPVP(const CLProtocol::MatchServerRaceEnd& raceEnd)
{
	...
	else if (raceEnd.type == PK_EQUAL_1V1 || raceEnd.type == PK_EQUAL_PRACTICE)  //公平竞技场结算暂时都按好友切磋处理
	{
		type = PK_PRACTICE;
	}
...
UInt32 Player::GetSignFlag()	//获取月签到标记位
{
...
bool Player::BoAlreadySign(UInt8 day)   //指定天数是否已签到
{
...
void Player::SetSign(UInt8 day)         //设置指定天数为已签到
{
	Avalon::Date date = GetLogicDate();
	if (day > date.GetDaysInMonth() || day < 1)
	{
...
bool Player::BoAlreadyCollectSign(UInt8 day)   //指定累计天数是否已领取
{
	Avalon::Date date = GetLogicDate();
	if (day > date.GetDaysInMonth() || day < 1)
	{
...
void Player::SetCollectSign(UInt8 day)         //设置指定累计天数为已领取
{
	Avalon::Date date = GetLogicDate();
	if (day > date.GetDaysInMonth() || day < 1)
	{
