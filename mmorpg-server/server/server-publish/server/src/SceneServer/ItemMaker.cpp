...
Item* ItemMaker::MakeItem(Player* player, UInt32 itemid, UInt16 num, UInt8 qualityLv, UInt8 strenth,
	UInt32 auctionCoolTimeStamp, UInt8 equipType, UInt8 enhanceType, bool isBuildWeapon, UInt32 auctionTransTimes)
{
	...
	if (equipType == EQUIP_HAVE_SMELL)
	{
		item->ChgEquipType(EQUIP_HAVE_SMELL);
	}
	else if (equipType == EQUIP_RED)
	{
		if (enhanceType == ENHANCE_INVALID)
		{
			ErrorLogStream << PLAYERINFO(player) << "EQUIP_RED Item must have EnhanceType!" << LogStream::eos;
			Item::DestroyItem(item);
			return NULL;
		}
		item->ChgEquipType(EQUIP_RED);
		item->ChgEnhanceType(static_cast<EnhanceType>(enhanceType));
		item->SetEnhanceValue(strenth);
	}
	else
	{
	...
	item->SetAuctionTransNum(auctionTransTimes);
	...
	//指定品级
	if (ItemPerfectGradeDataEntryMgr::Instance()->FindEntry(itemid))
	{
		item->SetQualityLv(100);
	}
	else if (qualityLv > 0)
	{
	...
	//设置装备,称号,时装属性
	if (!item->SetAttribute(isBuildWeapon))
	{
...
Item* ItemMaker::MakeItem(UInt32 itemid, UInt16 num, UInt8 qualityLv /*= 0*/, UInt8 strenth /*= 0*/, UInt32 auctionCoolTimeStamp /*= 0*/)
{
	if (num == 0) return NULL;
	...
	if (!item->SetDeadLine())
	{
		ErrorLogStream  << " Item SetDeadLine failed!" << LogStream::eos;
		Item::DestroyItem(item);
		return NULL;
	}
	...
	{
		ErrorLogStream << "Item SetAttribute failed!" << LogStream::eos;
		Item::DestroyItem(item);
		return NULL;
	}
