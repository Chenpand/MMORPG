...
public:
	...
	*/
	...
	UInt16 MoveItemToRoleStorage(Item* item, UInt16 num, UInt32 gridIndex);
	/**
	 *@brief 使用道具
	 */
	...
	// 检查能否添加道具
	bool CheckAddItem(UInt32 itemid, UInt16 num, ItemType itemType, UInt32 auctionCoolTime, UInt8 strengthen, UInt32 auctionTransTimes);

	// 得到给定数量的道具占用的格子数
	Int16 GetGridUsed(UInt32 itemid, UInt16 num, UInt32 auctionCoolTime, UInt8 strengthen, UInt32 auctionTransTimes);

	// 添加道具by itemid
	UInt16 AddItem(const char* reason, UInt32 itemid, UInt16 num, 
		UInt8 qualityLv, UInt8 strenth, ItemType itemType,
		UInt32 auctionCoolTimeStamp, UInt32 auctionTransTimes, UInt8 equipType, UInt8 enhanceType);
	UInt16 AddItemGetGuid(const char* reason, UInt32 itemid, UInt16 num,
		UInt8 qualityLv, UInt8 strenth, ItemType itemType, 
		UInt32 auctionCoolTimeStamp, UInt32 auctionTransTimes, std::vector<ObjID_t>& guids);
	...
	UInt32 GetItemNumWithFilter(UInt32 itemid, UInt8 pack, std::function<bool(Item*)>& filterCallback, UInt8 bindType = 0);
	...
	UInt16 RemoveItemWithFilter(const char* reason, UInt32 itemid, UInt16 num, UInt8 pack, std::function<bool(Item*)>& filterCallback);
	...
	//检查能否一键存仓库(只有仓库里有的才存)
	bool CheckOneKeyAddStorage(UInt32 itemId, UInt32 auctionCoolTimeStamp, UInt8 strengthen, UInt32 auctionTransTimes);
	//检查能否存角色仓库（按页存放，每页30个格子）
	bool CheckAddRoleStorage(Item* item, UInt16 num, UInt32 gridIndex);
	/**
	 *@brief 整理道具
	 */
	...
	//强化装备
	UInt32 StrengthenEquip(Item* equip, bool useUnbreak, ObjID_t strTicket, UInt32& probability, bool useDisposable);
	float GetStrenthRate(Item* equip, EquipStrRateData* sucRateData);
	//增幅装备
	UInt32 EnhanceEquip(Item* equip, bool useUnbreak, ObjID_t strTicket, UInt32& probability, bool useDisposable);
	float GetEnhanceRate(Item* equip, EquipStrRateData* sucRateData);



	//打造武器
	Item* BuildWeapon(UInt32 id, bool extraMaterial1, bool extraMaterial2);
	...
	*/
	...
	UInt32 ChangeEquip(Item* item, std::vector<Item*>& syncItems, UInt32 param1 = 0,  bool isChangeScheme = false);
	// 穿上/脱下时装
	...
	UInt32 GetFashionSize(UInt8 quality, ItemSubType subType, UInt8 isTrans);
	...
	UInt16 PushRoleStorage(Item* item, UInt16 num, UInt32 gridIndex);

	// 装备封装
	UInt32 CheckSealEquip(Item* item, UInt16& needNum, UInt32& needId);
	UInt32 SealEquip(Item* item, std::vector<UInt32>& inscriptionIds);
	...
	UInt32 ComposeMagicCard(Item* card_1, Item* card_2, UInt32& cardId, UInt32& cousumeBindGold, UInt32& consumeGold, UInt8& cardLev, bool autoUseGold = true, bool isonekey = false);
	...
	//宝珠转换
	void BeadConvert(UInt64 beadGuid, UInt64 materialGuid);
	...
	//背包附魔卡升级
	UInt32 UpgradeMagicCard(ObjID_t upgradeUid, ObjID_t materialUid, UInt32& cardTypeId, UInt8& cardLev, ObjID_t& cardGuid);
	//装备附魔卡升级
	UInt32 UpgradeMountMagicCard(ObjID_t equipUid, UInt32 cardId, ObjID_t materialUid, UInt32& cardTypeId, UInt8& cardLev);
	//一键合成附魔卡
	UInt32 ComposeMagicCardOneKey(UInt8 isCompWhite, UInt8 isCompBlue, UInt8 autoUseGold, UInt8 compNotBind,
		UInt8& endReason, UInt32& compTimes, UInt32& consumeBindGolds, UInt32& comsumeGolds, std::vector<ItemReward>& items);
	//合成附魔卡GM
	void ComposeMgaicGM(UInt32 card1, UInt32 card2, UInt32 num);
private:
...
public:
	...
	*/
	...
	//根据配置槽位寻找合适的药水类型
	std::vector<UInt32> GetFitPotionType(UInt32 type);

	void UseGiftPackGm(UInt32 itemId, UInt32 num);
private:
	/**
	*@brief 发送物品相关系统邮件
	*/
	 ...
	 *@brief 加道具到指定位置
	 */
	bool AddItem(Item* item, const ItemPos pos);
	...
	UInt32 UseEnhanceTicket(Item* equip, Item* ticket, UInt32& probability);
	...
	//检查道具使用次数上限
	UInt32 CheckItemUseLimitByDataId(Item* item);

	//道具使用后的次数检查
	void OnUsedLimitedItem(Item* item);

	//检查上限道具是否要销毁
	bool CheckLimitedItemDel(Item* item);

	//使用团本道具
	UInt32 UseTeamCopyItem(Item* item, ItemDataEntry* itemData);

public: //装备装换
	UInt32 HandleEquipConvert(UInt8 type, UInt64 soucreEqGuid, UInt32 targetEqTypeId, UInt64 convertorGuid, UInt64& eqGuid);
	UInt32 HandleEquipConvertSomeSuit(UInt8 type, UInt64 soucreEqGuid, UInt32 targetEqTypeId, UInt64 convertorGuid, UInt64& eqGuid);
	UInt32 HandleEquipConvertDiffSuit(UInt8 type, UInt64 soucreEqGuid, UInt32 targetEqTypeId, UInt64 convertorGuid, UInt64& eqGuid);
public://编解码
