...
void GMCmdHandler::TransmitToTeamCopy(ObjID_t playerId, const CLParameter& params)
{
	CLProtocol::SysTransmitGmCommand req;
 ...
 *@brief 加1v1PK场数
 */
class AddPKSeason1V1CmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "add1v1"; }
	const char* GetHelp() const { return " num=xx"; }
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		...
		while (num--)
		{
			player->GetPkStatisticMgr().OnUpdatePkStatistic(PK_SEASON_1V1, OCCU_NONE, PkRaceResult::PKRR_WIN);
		}
		return true;
	}
};
...
class AddChallengeScoreCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "addcs"; }
	const char* GetHelp() const { return " num=xx"; }

	bool Execute(Player* player, const CLParameter& params)
	{
		UInt32 num = params.GetParam("num");
		player->GetActiveTaskMgr().OnChallengeScoreAdd(num);
		return true;
	}
};

class SetChampionCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "us"; }
...
class ChampionCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "champion"; }
	const char* GetHelp() const { return " op=xx"; }

	bool Execute(Player* player, const CLParameter& params)
	{
		UInt32 op = params.GetParam("op");
		switch (op)
		{
		case 3 :
		{
			CLProtocol::SceneChampionEnrollReq req;
			HandleProtocol(player, req);
			break;
		}
		case 4 :
		{
			CLProtocol::SceneChampionJoinPrepareReq req;
			HandleProtocol(player, req);
			break;
		}
		case 5:
...
class AddSpringScoreCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "addss"; }
	const char* GetHelp() const { return " num=xx"; }

	bool Execute(Player* player, const CLParameter& params)
	{
		UInt32 num = params.GetParam("num");
		player->GetActiveTaskMgr().OnSpringScoreAdd(num);
		return true;
	}
}; 

class AddAccountOnlineCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "acconline"; }
	const char* GetHelp() const { return " num=xx"; }
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		...
		{
			player->GetActiveTaskMgr()._UpdateAccountCumulateOnlineOpActTask();
		}
		return true;
	}
};
 ...
 *@brief 加信用点券
 */
class AddCreditPointCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "addcpoint"; }
	const char* GetHelp() const { return " num=xx"; }

	bool Execute(Player* player, const CLParameter& params)
	{
		UInt32 num = params.GetParam("num");
		player->AddCreditPoint(GetReason(SOURCE_TYPE_GM).c_str(), num);
		CreditPointRecordMgr::Instance()->ObtainCreditPoint(player->GetAccID(), num, 0);
		return true;
	}
};
 ...
 *@brief 删除信用点券
 */
class RemoveCreditPointCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "removecpoint"; }
	const char* GetHelp() const { return " num=xx"; }

	bool Execute(Player* player, const CLParameter& params)
	{
		UInt32 num = params.GetParam("num");
		player->RemoveCreditPoint(GetReason(SOURCE_TYPE_GM).c_str(), num);
		return true;
	}
};
...
*@brief 加增幅道具
*/
class AddEnhanceEquipCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const{ return "addee"; }
	const char* GetHelp() const { return "id=xxxx num=xxx ql=xxx str=xxx et=xxx ent=xxx"; }
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		...
		UInt8 ql = params.GetParam("ql");
		UInt8 strengthen = params.GetParam("str");
		UInt8 equipType = params.GetParam("et");
		UInt8 enhanceType = params.GetParam("ent");
		std::string reason = GetReason(LogSourceType::SOURCE_TYPE_GM);

		if (player->CheckAddItem(id, num))
		{
			if (num == player->AddEnhanceEquip(GetReason(SOURCE_TYPE_GM).c_str(), id, num, ql, strengthen, true, static_cast<EquipType>(equipType), static_cast<EnhanceType>(enhanceType)))
			{
				return true;
			}
		}
...
//增幅模拟测试
class EnhanceSimCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "enhance"; }
	const char* GetHelp() const { return "times = xx b = xx e = xx lv = xx q = xx"; }

	bool Execute(Player* player, const CLParameter& params)
	{
		UInt32 times = params.GetParam("times");
		UInt32 beginEnhanceLv = params.GetParam("b");
		UInt32 endEnhanceLV = params.GetParam("e");
		UInt32 equipLv = params.GetParam("lv");
		UInt32 quailty = params.GetParam("q");
		UInt32 tmptimes = times;

		//参数合法性
		if (endEnhanceLV > ITEM_STRENTH_MAX)
		{
			return false;
		}
		if (equipLv > 60)
		...
		vstr.push_back("times,costTimes,costGold,costXKWL\n");
		...
		while (times--)
		{
			...
			UInt32 costTimes = 0;
			UInt32 costXKWL = 0;
			UInt32 costBindGold = 0;

			player->SimEnhance(beginEnhanceLv, endEnhanceLV, equipLv, quailty, costXKWL, costBindGold, costTimes);

			std::stringstream s;
			s << times << ',' << costTimes << ',' << costBindGold << ',' << costXKWL << std::endl;
			vstr.push_back(s.str());

			//Protocol::SceneSyncChat protocol;
		...
		std::string rootpath = SSApplication::Instance()->GetEnhanceCountPath();
		std::string accidStr = Avalon::StringUtil::ConvertToString(player->GetAccID());
		std::string timesStr = Avalon::StringUtil::ConvertToString(tmptimes);
		std::string bStr = Avalon::StringUtil::ConvertToString(beginEnhanceLv);
		std::string eStr = Avalon::StringUtil::ConvertToString(endEnhanceLV);

		std::string postfix = accidStr + "_" + timesStr + "_" + bStr + "_" + eStr + ".csv";
		std::string filename = rootpath + "EnhanceCount_" + postfix;
...
//铭文合成模拟测试
class EquipInscriptionSynthesisSimCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "mwhc"; }
	const char* GetHelp() const { return "times=xx color=xx num=xx"; }
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		...
		std::vector<std::string> vstr;
		vstr.push_back("id,name,num\n");

		ItemRewardVec result;
		player->EquipInscriptionSynthesisSim(result, times, color, num);

		for (auto &i : result)
		{
			auto data = ItemDataEntryMgr::Instance()->FindEntry(i.id);
			if (!data)
			{
				vstr.emplace_back(std::to_string(i.id) + ',' + "null" + ',' + std::to_string(i.num) + '\n');
				continue;
			}
			vstr.emplace_back(std::to_string(i.id) + ',' + data->name + ',' +std::to_string(i.num) + '\n');
		}

		std::string rootpath = SSApplication::Instance()->GetEnhanceCountPath();
		std::string accidStr = Avalon::StringUtil::ConvertToString(player->GetAccID());
		std::string timesStr = Avalon::StringUtil::ConvertToString(times);
		std::string bStr = Avalon::StringUtil::ConvertToString(color);
		std::string eStr = Avalon::StringUtil::ConvertToString(num);

		std::string postfix = accidStr + "_" + timesStr + "_" + bStr + "_" + eStr + ".csv";
		std::string filename = rootpath + "mwhc_" + postfix;
...
//时装分解模拟测试
class FashionDecomposeSimCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "fashionDecompose"; }
	const char* GetHelp() const { return "times=xx fashionId=xx"; }

	bool Execute(Player* player, const CLParameter& params)
	{
		UInt32 times = params.GetParam("times");
		UInt32 fashionId = params.GetParam("fashionId");

		ItemDataEntry* dataEntry = ItemDataEntryMgr::Instance()->FindEntry(fashionId);
		if (!dataEntry)
		{
			return false;
		}

		UInt32 key = FashionDecomposeDataEntry::MakeKey(dataEntry->subType, dataEntry->thirdType, dataEntry->color);
		...
		{
			ItemRewardVec temp;
			InscriptionDropItemMgr::Instance()->GenDropItems(decomposeDataEntry->groupID, player->GetOccu(), temp);
			for (auto &i : temp)
			{
		...
		for (auto itr : resultMap)
		{
			...
			if (!data)
			{
				vstr.emplace_back(std::to_string(itr.first) + ',' + "null" + ',' + std::to_string(itr.second) + '\n');
				continue;
			}

			vstr.emplace_back(std::to_string(itr.first) + ',' + data->name + ',' + std::to_string(itr.second) + '\n');
		}

		std::string rootpath = SSApplication::Instance()->GetJarCountPath();
		std::string accidStr = Avalon::StringUtil::ConvertToString(player->GetAccID());
		std::string timesStr = Avalon::StringUtil::ConvertToString(times);
		std::string bStr = Avalon::StringUtil::ConvertToString(fashionId);

		std::string postfix = accidStr + "_" + timesStr + "_" + bStr + ".csv";
		std::string filename = rootpath + "fashionDecompose_" + postfix;
...
public:
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		...
		player->StrengthenEquip(uid, (useUnbreak != 0), 0, false);
...
public:
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		...
		if (color >= ITEM_QUALITY_MAX) return false;
...
public:
	...
	{
	public:
		Task(Player* owner, UInt32 type, UInt32 combo, UInt32 num) : owner(owner), type(type), combo(combo), num(num)
		{
		}
		...
		virtual void Run()
		{
			InfoLogStream << "OpenJarSampleCmdHandler task!" << LogStream::eos;
			owner->OpenJarSample(type, combo, num);
			m_runing = 0;
			return;
		}
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		InfoLogStream << "enter OpenJarSampleCmdHandler::Execute" << LogStream::eos;
		if (m_runing == 1)
		{
			player->SendNotify("can't run multi this gm cmd!");
			return true;
		}
...
public:
	static UInt8 m_runing;
};

UInt8 OpenJarSampleCmdHandler::m_runing = 0;
...
public:
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		...
		player->OpenJar(type, combo, opActId, 0);
...
public:
	...
	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		...
		if (type == "all")
		{
			...
			pPlayer->VisitItems(PACK_INSCRIPTION, visitor);
		}
		else
		{
...
class WarpStoneUnlockCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "wsunlock"; }
	const char* GetHelp() const { return "id=xxxx  //warp stone unlock"; }

	bool Execute(Player* player, const CLParameter& params)
	{
		UInt32 id = params.GetParam("id");
		if (player->WarpStoneUnlock(id) == ErrorCode::SUCCESS)
			return true;
		return false;
	}
};

class WarpStoneChargeCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "wscharge"; }

	bool Execute(Player* player, const CLParameter& params)
	{
		UInt32 id = params.GetParam("id");
		std::vector<ChargeInfo> chargeInfo;
		std::string sidStr = params.GetParam("sid").GetString();
		std::string numStr = params.GetParam("num").GetString();
		std::vector<std::string> sidVec;
		std::vector<std::string> numVec;
		Avalon::StringUtil::Split(sidStr, sidVec, "|");
		Avalon::StringUtil::Split(numStr, numVec, "|");
		UInt32 size = sidVec.size();
		size = size < numVec.size() ? size : numVec.size();
		for (UInt32 i = 0; i < size;++i)
		{
			ChargeInfo info;
			info.itemId = Avalon::StringUtil::ConvertToValue<UInt32>(sidVec[i]);
			info.num = Avalon::StringUtil::ConvertToValue<UInt32>(numVec[i]);
			chargeInfo.push_back(info);
		}

		if (player->WarpStoneCharge(id, chargeInfo) == ErrorCode::SUCCESS)
			return true;
		return false;
	}
};

class WarpStoneMinLevelCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "wsminlevel"; }
	const char* GetHelp() const { return " //is debug"; }

	bool Execute(Player* player, const CLParameter& params)
	{
		/*
		UInt32 level = params.GetParam("level");
		UInt32 size = params.GetParam("size");

		UInt32 num = player->GetWarpStoneMinLevel(level, size);
		*/
		return true;
	}
...
class WarpStoneUpLevelCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "wsuplevel"; }
	const char* GetHelp() const { return " // id=xx level=xx"; }
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		...
		player->WarpStoneUpLevel(id, level);
		return true;
	}
};
...
public:
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		player->GetActiveTaskMgr().OnDayChange();
		return true;
	}
};
...
class ClearMonthCardCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "clearmonthcard"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		pPlayer->InitMonthCardExpireTime(0);
		pPlayer->OnMonthCardWelfare(0, false);
		pPlayer->SyncBaseInfoToWorld();
...
class AddMonthSignCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const{ return "sign"; }
	const char* GetHelp() const { return "day=xxx"; }

	bool Execute(Player* player, const CLParameter& params)
	{
		UInt32 day = params.GetParam("day");

		if (player->SignMonth(day) == ErrorCode::SUCCESS)
		{
			return true;
		}
		return false;
...
public:
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		...
		player->GetActiveTaskMgr().SubmitTaskRp(taskIds);

		return true;
	}
};

class SetDayOnlineTimeCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "online"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		UInt32 time = params.GetParam("time");
		pPlayer->GetAntiAddictMgr().SetAntiAddictTime(time);
		pPlayer->GetAntiAddictMgr().SyncAddictTimeToCenter();
		DebugLogStream << PLAYERINFO(pPlayer) << " set online time: " << time << LogStream::eos;
		return true;
	}
};

class SetPlayerAgeCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "setAge"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		UInt8 age = params.GetParam("age");

		pPlayer->GetAntiAddictMgr().SetAge(age);

		DebugLogStream << PLAYERINFO(pPlayer) << "setAge(" << age << ")" << LogStream::eos;
		return true;
	}
};
...
public:
	...
	{
		UInt32 num = params.GetParam("num");
		pPlayer->GetActiveTaskMgr().OnUsedHellTicket(num);
		return true;
	}
};
...
public:
	...
	{
		UInt16 num = params.GetParam("num");
		pPlayer->GetActiveTaskMgr().OnRemoveFatigue(num);
		return true;
	}
};
...
public:
	...
	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		UInt32 actId = params.GetParam("actId");
		UInt32 taskId = params.GetParam("taskId");
		UInt64 param = params.GetParam("param");

		pPlayer->GetActiveTaskMgr().OnTakeOpActTaskReward(actId, taskId, param);
...
class StartDungeomCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "dungeon"; }
	...
	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		...
		if (pPlayer->GetTeam())
		{
			pPlayer->GetTeam()->OnPlayerRequestStartDungeonRace(pPlayer, id, cityMonster);
			return true;
		}

		CLProtocol::WorldMonopolyGMJoinDungeon req;
		req.roleId = pPlayer->GetID();
		req.dungeonId = id;
		Router::SendToWorld(req);


		pPlayer->GetDungeonMgr().OnStartDungeonReq(id, std::vector<UInt32>(), cityMonster);
...
class ScoreWar2V2ChangeStatusHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "2v2"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		ScoreWar2V2Status status = (ScoreWar2V2Status)((UInt32)params.GetParam("status"));

		CLProtocol::SceneCross2V2GMSync sync;
		sync.isByTime = false;
		sync.targetStatus = status;
		Router::SendToCross(sync);
...
public:
	...
	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		...
		pPlayer->IncCounter(DAILY_ACTIVE_SCORE, num);
		return true;
	}
};
...
public:
	...
	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		...
		UInt32 num = params.GetParam("num");
		UInt32 isTeam = params.GetParam("team");
		if (num == 0)
		{
			pPlayer->GetActiveTaskMgr().GMCommandClearDungeon(dungeonId, isTeam > 0);
		}
...
class SetItemDepositCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "itemdp"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		ItemReward reward;
		reward.id = params.GetParam("id");
		reward.num = params.GetParam("num");
		reward.strenth = params.GetParam("st");
		reward.equipType = params.GetParam("eq");
		pPlayer->GetItemDepositMgr().AddItemDeposit(reward);

		return true;
	}
};

class SetLimitTimeHellCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "lth"; }
	...
	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		...
		if (num > 0)
		{
			pPlayer->GetActiveTaskMgr().GMLimitTimeHellNum(num);
		}
...
class SetWeekSignCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "ws"; }
	...
	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		...
		if (num > 0)
		{
			pPlayer->GetActiveTaskMgr().GMWeekSignNum(num);
		}

		UInt32 rt = params.GetParam("rt");
		if (rt == 1)
		{
			pPlayer->GetActiveTaskMgr().GMResetWeekSignTask();
		}
		else if (rt == 2)
		{
			pPlayer->GetActiveTaskMgr().GMFinishWeekSignTask();
		}
		return true;
	}
};

class SetWeekSignRateCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "wsrate"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		UInt32 vip = params.GetParam("vip");
		UInt32 num = params.GetParam("num");
		if (num >= 10000)
			num = 10000;
		...
		while (num--)
		{
			...
			req.actId = 0;
			req.vipLvl = vip;
			pPlayer->SendToGlobalActivity(req);
		}
...
class SetTeamCopyCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "teamcopy"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		if (NULL == pPlayer) return false;

		TransmitToTeamCopy(pPlayer->GetID(), params);
		return true;
	}
};

class CalcRoleValueScoreCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "crvs"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		pPlayer->CalcRoleValueScore(true);

		return true;
	}
};

class SetPlayerDayOnlineTimeCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "setDayOnlineTm"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		
		UInt32 time = params.GetParam("time");
		pPlayer->SetDayOnlineTimeGm(time);
		return true;
	}
};

class SetDungeonChapterCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "setDungChap"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{

		UInt32 chapter = params.GetParam("chapter");
		pPlayer->GetDungeonMgr().SetChapterPass(chapter);
		return true;
	}
};

class SetAllChapterDungeonPassedCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "passDungChap"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		pPlayer->GetDungeonMgr().PassAllChapterDungeon();
		return true;
	}
};

class SendMatch2V2CmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "send2v2"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{

		/*CLProtocol::MatchServerMatchStartReq msReq;
		msReq.type = PK_2V2_ACTIVITY;
		msReq.info.id = 1;
		msReq.info.accid = 1;
		msReq.dailyCount = 0;
		msReq.seasonLevel = 1;
...
class Set2V2ScoreHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "2v2score"; }
	...
	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		...
		pPlayer->Set2V2MatchScore(score);

		return true;
	}
};

class WeaponBuildGMHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "wb"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{

		UInt32 id = params.GetParam("id");
		UInt32 extra_1 = params.GetParam("ex1");
		UInt32 extra_2 = params.GetParam("ex2");
		...
		auto weapon = pPlayer->GetItemMgr().BuildWeapon(id, extra_1, extra_2);
		if (weapon != nullptr)
		{
			pPlayer->AddItemDirect(GetReason(SOURCE_TYPE_GM).c_str(), weapon);
		}
...
class ActiveRecordCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "setRecord"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		UInt32 role = params.GetParam("role");
		UInt32 opActId = params.GetParam("opActId");
		UInt32 counterId = params.GetParam("counterId");
		UInt32 value = params.GetParam("value");
		OpActivityBehavior behavior = (OpActivityBehavior)counterId;

		if (role != 0)
		{
			OpActivityRecordMgr::Instance()->SetRecord(behavior, OP_ACTIVITY_BEHAVIOR_OWNER_ROLE, pPlayer->GetID(), opActId, value);
		}
		else
		{
			OpActivityRecordMgr::Instance()->SetRecord(behavior, OP_ACTIVITY_BEHAVIOR_OWNER_ACCOUNT, pPlayer->GetAccID(), opActId, value);
		}
...
class ChangeSchemeCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "changeScheme"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		UInt8 type = params.GetParam("type");
		UInt32 schemeId = params.GetParam("schemeId");
		UInt8 isSync = params.GetParam("isSync");
		std::vector<UInt64> overdueIds;
		pPlayer->GetEqSchemeMgr().OnPlayerWearReq(type, schemeId, isSync, overdueIds);

		return true;
	}
};

class HonorRankCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "honorRank"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		GameParamMgr::Instance()->SetValue(HONOR_RANK_TIME, 0);
		return true;
	}
};

class HonorLevelCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "honorLvl"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		if (NULL == pPlayer) return false;

		UInt32 level = params.GetParam("lvl");
		HonorMgr::Instance()->UpdateHonorLevel(pPlayer->GetID(), level);
		return true;
	}
};

class BeadConvertCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "beadConvert"; }
	...
	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		...
		DebugLogStream << PLAYERINFO(pPlayer) << "bead convert dropId:" << dropId << " num:" << num << LogStream::eos;

		std::string startStr;
		startStr.append("bead convert dropId:").append(std::to_string(dropId))
			.append(" num:").append(std::to_string(num)).append("\n");
		std::vector<std::string> beadStrVec;
		beadStrVec.push_back(std::move(startStr));

		std::string rootpath = SSApplication::Instance()->GetBeadConvertCountPath();
		std::string filename;
		filename.reserve(256);
		filename.append(rootpath).append("beadConvert").append("_")
			.append(std::to_string(pPlayer->GetAccID())).append("_")
			.append(std::to_string(dropId)).append("_").append(std::to_string(num)).append(".txt");

		std::map<UInt32, UInt32> itemMap;
		while (num--)
		{
			BeadConvertDropDataEntryMgr::Instance()->GetConvertReward(dropId, itemId, itemNum);
			itemMap[itemId] += itemNum;
		}

		for (auto& iter : itemMap)
		{
			DebugLogStream << PLAYERINFO(pPlayer) << "bead convert itemId:" << iter.first << " itemNum:" << iter.second << LogStream::eos;
			std::string aa;
			aa.reserve(256);
			aa.append("bead convert itemId:").append(std::to_string(iter.first))
				.append(" itemNum:").append(std::to_string(iter.second)).append("\n");
			beadStrVec.push_back(std::move(aa));
		}
...
class SetLostDungeonCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "SetLostDungSt"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		if (NULL == pPlayer) return false;

		UInt8 st = params.GetParam("st");
		pPlayer->SetCounter(COUNTER_LOSTDUNG_STATE, st);
		return true;
	}
};

class PlantAppraStatisticCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "plantStatis"; }


	class Task : public Avalon::Thread
	{
	public:
		Task(Player* owner_, UInt32 prof_, UInt32 times_) : owner(owner_), prof(prof_), times(times_)
		{
		}
		...
		virtual void Run()
		{
			if (owner)
			{
				owner->StatisticPlantAppra(prof, times);
			}
			return;
		}
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		UInt32 prof = params.GetParam("prof");
		UInt32 times = params.GetParam("times");

		auto thread = new Task(player, prof, times);
		thread->Activate();
		return true;
	}
};

class SettleLostDungCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "SettleLostDung"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		if (NULL == pPlayer) return false;
		UInt32 floor = params.GetParam("floor");
		UInt8 result = params.GetParam("result");
		UInt32 score = params.GetParam("score");
		pPlayer->GetLostDungeon().SettleFloor(floor, result, score);
		return true;
	}
};

class CreditPointTransferCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "cpt"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		if (NULL == pPlayer) return false;
		CreditPointRecordMgr::Instance()->GMTransferCreditPoint(pPlayer->GetAccID());
		return true;
	}
};

class SetCreditPointMonthGetCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "cpmg"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		if (NULL == pPlayer) return false;
		UInt32 num = params.GetParam("num");
		OpActivityRecordMgr::Instance()->SetRecord(OP_ACTIVITY_BEHAVIOR_CREDIT_POINT_MONTH_GET,
			OP_ACTIVITY_BEHAVIOR_OWNER_ACCOUNT, pPlayer->GetAccID(), 0, num);
		pPlayer->InitCreditPointMonthGet(num);
		pPlayer->SyncSelfData(true);
		return true;
	}
};

class GoldConsignmentCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "goldcon"; }
	...
	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		...
		req.cmdname = GetCmdName();
		req.params = params;
		Router::SendToUnion(USFT_GOLD_CONSIGNMENT, req);
		return true;
	}
};

class ComposeMagicCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "compMagic"; }

	class Task : public Avalon::Thread
	{
	public:
		Task(Player* owner, UInt32 c1, UInt32 c2, UInt32 num)
			: owner(owner), card1(c1), card2(c2), num(num)
		{
		}
		...
		virtual void Run()
		{
			InfoLogStream << "ComposeMagicCmdHandler task!" << LogStream::eos;
			owner->GetItemMgr().ComposeMgaicGM(card1, card2, num);
			m_runing = 0;
			return;
		}
	...
	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		...
		if (m_runing == 1)
		{
			pPlayer->SendNotify("can't run multi this gm cmd!");
			return true;
		}

		m_runing = 1;

		UInt32 card1 = params.GetParam("c1");
		UInt32 card2 = params.GetParam("c2");
		UInt32 num = params.GetParam("num");
		
		auto thread = new Task(pPlayer, card1, card2, num);
		thread->Activate();
		return true;
	}
...
UInt8  ComposeMagicCmdHandler::m_runing = 0;

class OpenGiftCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "OpenGift"; }

	class Task : public Avalon::Thread
	{
	public:
		Task(Player* owner_, UInt32 itemId_, UInt32 num_)
			: owner(owner_), itemId(itemId_), num(num_)
		{
		}
		...
		virtual void Run()
		{
			InfoLogStream << "OpenGiftCmdHandler task!" << LogStream::eos;
			owner->GetItemMgr().UseGiftPackGm(itemId, num);
			m_runing = 0;
			return;
		}
	...
	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		...
		auto thread = new Task(pPlayer, itemId, num);
		thread->Activate();
...
UInt8  OpenGiftCmdHandler::m_runing = 0;

class SetZjslTowerFloorCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "set_zjsl_floor"; }

	bool Execute(Player* pPlayer, const CLParameter& params)
	{
		UInt32 floor = params.GetParam("num");

		pPlayer->SetCounter(COUNTER_ZJSL_TOP_FLOOR, floor);
		pPlayer->SetCounter(COUNTER_ZJSL_CHALLENGE_TIMES, 0);
		pPlayer->SetCounter(COUNTER_ZJSL_DUNGEON_TIMES, 0);

		pPlayer->SetCounter(COUNTER_ZJSL_DUNGEON_BUFF, 0);
		pPlayer->SetCounter(COUNTER_ZJSL_DUNGEON_BUFF_TARGET, 0);
		pPlayer->SetCounter(COUNTER_ZJSL_DUNGEON_BUFF_FLOOR, 0);
		pPlayer->SetCounter(COUNTER_ZJSL_INSPIRE_BUFF, 0);
		pPlayer->SetCounter(COUNTER_ZJSL_INSPIRE_BUFF_DAY, 0);

		pPlayer->SetCounter(COUNTER_ZJSL_PLAYER_HP, 0);
		pPlayer->SetCounter(COUNTER_ZJSL_PLAYER_MP, 0);

		pPlayer->SetCounter(COUNTER_ZJSL_BOSS1_ID, 0);
		pPlayer->SetCounter(COUNTER_ZJSL_BOSS2_ID, 0);
		pPlayer->SetCounter(COUNTER_ZJSL_BOSS1_HP, 0);
		pPlayer->SetCounter(COUNTER_ZJSL_BOSS2_HP, 0);

		return true;
	}
};

extern UInt32 TASK_VISIT_NUM;
extern UInt32 TASK_CALL_SCRIPT_NUM;

/**
*@brief test
*/
class TestCmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "test"; }
	const char* GetHelp() const { return "num=xx"; }

	bool Execute(Player* player, const CLParameter& params)
	{
		TASK_VISIT_NUM = 0;
		TASK_CALL_SCRIPT_NUM = 0;

		UInt32 num = params.GetParam("num");

		auto startTime = PlayerMgr::Instance()->GetUSecsNow();
		for (int i = 0; i < num; i++)
		{
			player->GetAchievementTaskMgr().OnGetItem(1, 1, 1, 1);
			player->GetTaskMgr().OnGetItem(1, 1, 1, 1);
		}
		auto endTime = PlayerMgr::Instance()->GetUSecsNow();
		InfoLogStream << "times: " << num << " used " << endTime - startTime << "(us) visit(" << TASK_VISIT_NUM << ") call script(" << TASK_CALL_SCRIPT_NUM << ")" << LogStream::eos;
		return true;
	}
};
...
*@brief test
*/
class Test2CmdHandler : public GMCmdHandler
{
public:
	const char* GetCmdName() const { return "test2"; }
	const char* GetHelp() const { return "id= xx num=xx"; }
	...
	bool Execute(Player* player, const CLParameter& params)
	{
		...
		for (int i = 0; i < num; i++)
		{
			ScriptMgr::Instance()->FindTaskScript(id);
		}
		auto endTime = PlayerMgr::Instance()->GetUSecsNow();
		InfoLogStream << "times: " << num << " used " << endTime - startTime << "(us)" << LogStream::eos;
		return true;
	}
};
...
void GMSystem::RegisterHandlers()
{
	...
	RegisterHandler(GM_AUTHORITY_SUPER, new AddPKSeason1V1CmdHandler());
	...
	RegisterHandler(GM_AUTHORITY_SUPER, new AddCreditPointCmdHandler());
	RegisterHandler(GM_AUTHORITY_SUPER, new RemoveCreditPointCmdHandler());
	...
	RegisterHandler(GM_AUTHORITY_SUPER, new AddMonthSignCmdHandler());

	

	// item
	RegisterHandler(GM_AUTHORITY_SUPER, new AddItemCmdHandler());
	RegisterHandler(GM_AUTHORITY_SUPER, new AddEnhanceEquipCmdHandler());
	RegisterHandler(GM_AUTHORITY_SUPER, new EquipInscriptionSynthesisSimCmdHandler());
	RegisterHandler(GM_AUTHORITY_SUPER, new EnhanceSimCmdHandler());
	RegisterHandler(GM_AUTHORITY_SUPER, new FashionDecomposeSimCmdHandler());
	...
	RegisterHandler(GM_AUTHORITY_SUPER, new AddChallengeScoreCmdHandler());
	RegisterHandler(GM_AUTHORITY_SUPER, new AddSpringScoreCmdHandler());
	RegisterHandler(GM_AUTHORITY_SUPER, new AddAccountOnlineCmdHandler());
	RegisterHandler(GM_AUTHORITY_SUPER, new ChampionCmdHandler());
	RegisterHandler(GM_AUTHORITY_SUPER, new SetChampionCmdHandler());
	...
	//WarpStone  adder by huchenhui 2016.07.21
	RegisterHandler(GM_AUTHORITY_DEBUG, new WarpStoneUnlockCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new WarpStoneChargeCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new WarpStoneMinLevelCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new WarpStoneUpLevelCmdHandler());
	...
	RegisterHandler(GM_AUTHORITY_DEBUG, new ClearMonthCardCmdHandler());
	...
	RegisterHandler(GM_AUTHORITY_DEBUG, new SetDayOnlineTimeCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new SetPlayerAgeCmdHandler());
	...
	RegisterHandler(GM_AUTHORITY_DEBUG, new StartDungeomCmdHandler());
	...
	RegisterHandler(GM_AUTHORITY_DEBUG, new ScoreWar2V2ChangeStatusHandler());
	...
	RegisterHandler(GM_AUTHORITY_DEBUG, new SetItemDepositCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new SetLimitTimeHellCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new SetWeekSignCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new SetWeekSignRateCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new SetTeamCopyCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new CalcRoleValueScoreCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new SetPlayerDayOnlineTimeCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new SetDungeonChapterCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new Set2V2ScoreHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new WeaponBuildGMHandler());
	
	RegisterHandler(GM_AUTHORITY_DEBUG, new ActiveRecordCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new ChangeSchemeCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new SetAllChapterDungeonPassedCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new HonorRankCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new HonorLevelCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new BeadConvertCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new SetLostDungeonCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new PlantAppraStatisticCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new SettleLostDungCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new CreditPointTransferCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new SetCreditPointMonthGetCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new GoldConsignmentCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new ComposeMagicCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new OpenGiftCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new SetZjslTowerFloorCmdHandler());

	RegisterHandler(GM_AUTHORITY_DEBUG, new TestCmdHandler());
	RegisterHandler(GM_AUTHORITY_DEBUG, new Test2CmdHandler());
}
