...
private: //新月签到相关
	void OnSign();          //签到成功
public:
	UInt32 GetSignFlag();	//获取月签到标记位
	UInt32 GetSignCollectFlag();     //获取月累计签到奖励标记位
	bool BoAlreadySign(UInt8 day);   //指定天数是否已签到
	void SetSign(UInt8 day);         //设置指定天数为已签到
	bool BoAlreadyCollectSign(UInt8 day);   //指定累计天数的奖励是否已领取
	void SetCollectSign(UInt8 day);         //设置指定天数为已签到
	UInt32 SignMonth(UInt8 day);     //月签到
	void SendSignStatus();           //发送月签到情况
	UInt32 SignCollect(UInt8 day);   //累计签到奖励领取  
	void OnActiviteAdd();            //活跃度增加到一定值就增加活跃签到次数
	UInt32 SignMonthAll(UInt8 day);  //全部补签
	Avalon::Date GetLogicDate();     //获取逻辑上的日期每天6点前算前一天，6点后算后一天
...
public://任务相关
	...
	*/
	...
	/**
	*@brief 礼遇特权状态同步到World
	*/
	void SyncGiftRightToWorld();	
	
	/**
	*@brief 深渊特权状态同步到World
	*/
...
public://事件
	...
	*/
	...
	/**
	*@brief 次元石解锁事件 Adder by huchenhui 2016.11.09
	*/
	void OnWarpStoneUnlock(WarpStoneDataEntry* dataEntry);

	/**
	*@brief 次元石升级事件 Adder by huchenhui 2016.08.23
	*/
	void OnWarpStoneUpLevel(WarpStoneDataEntry* dataEntry, UInt32 level);

	/**
	*@brief 次元石充能事件 Adder by huchenhui 2016.09.09
	*/
...
public:	//植树节
	...
	UInt32	m_plantGrowEndTm;	//种植结束时间戳
