...
void HandleProtocol(Player* player, CLProtocol::SceneGiftPackInfoReq& req)
{
	...
	for (auto giftPackId : req.giftPackIds)
	{
		...
		for (auto giftId : giftPackData->giftItems)
		{
			...
			ItemDataEntry* itemData = ItemDataEntryMgr::Instance()->FindEntry(giftData->itemId);
			if (!itemData)
			{
				ErrorLogStream << PLAYERINFO(player) << "Can not find item(" << giftData->itemId << ")!" << LogStream::eos;
				continue;
			}
			...
			gift.equipType = giftData->equipType;
			gift.strengthen = giftData->strengthen;
			gift.isTimeLimit = itemData->time > 0 || itemData->expireTime > 0;
