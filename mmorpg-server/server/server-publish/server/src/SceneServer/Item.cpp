...
Item::Item()
{
	...
	m_BeadReplaceCnt = 0;
	m_ItemScore = 0;
	m_EquipType = EQUIP_NORMAL;
	m_EnhanceType = ENHANCE_INVALID;
	m_EnhanceNum = 0;
	m_EnhancePVPNum = 0;
	m_IndependAtkStreng = 0;
	m_IndependAtkPvpStreng = 0;
	m_pAttrDataInherent = NULL;
	m_AttrDataInherentId = 0;
	m_AuctionTransNum = 0;
}
...
UInt32 Item::GetEquipGrid()
{
	...
	{
		if (dataEntry->subType >= ST_ASSIST_EQUIP && dataEntry->subType <= ST_EARRINGS)
		{
			return dataEntry->subType - 88;
		}
		else
		{
...
UInt16 Item::CheckMerging(UInt16 num, UInt32 auctionCoolTime, UInt8 strengthen, UInt32 auctionTransTimes)
{
	...
	if (m_Strengthen != strengthen)
	{
		return 0;
	}

	if (m_AuctionTransNum != auctionTransTimes)
	{
		return 0;
	}
...
UInt16 Item::Merge(const char* reason, Player* player, UInt16 num, UInt32 auctionCoolTimeStamp, UInt8 strengthen, UInt32 auctionTransTimes)
{
...
void Item::GetRaceEquip(RaceEquip& equip, bool pvp)
{
	...
	equip.equipType = GetEquipType();
	equip.enhanceType = GetEnhanceType();
	...
	if (pvp)
	{
		...
		equip.enhanceNum = m_EnhancePVPNum;
		if (IsAssistEquip(GetSubType()))
		{
			equip.strenth = GetPVPAssistStrength();
			equip.stamina = GetPVPAssistStamina();
			equip.intellect = GetPVPAssistIntellect();
			equip.spirit = GetPVPAssistSpirit();
		}

		equip.independAtk = GetPVPIndependAtk();
		equip.indpendAtkStreng = GetPVPIndependAtkStreng();
	}
	else
	{
		...
		equip.enhanceNum = m_EnhanceNum;
		equip.independAtk = GetIndependAtk();
		equip.indpendAtkStreng = GetIndependAtkStreng();
	}
	...
	equip.magicCardLv = m_AddMagic.GetMagicCardLev();
	...
	{
		auto attrData = GetAttrDataEntry();
		equip.atkPropEx.push_back(attrData ? attrData->AtkPropEx[i] : 0);
	...
	m_InscriptionHoles.GetRaceInfo(equip.inscriptionIds);
}
...
bool Item::SetStrengthen(UInt8 strengthen)
{
	...
	if (data->subType == ST_MAGICCARD)
	{
		MagicCardData* cardData = MagicCardDataMgr::Instance()->FindEntry(data->GetKey());
		if (!cardData || cardData->maxLevel == 0
			|| strengthen > cardData->maxLevel)
		{
			return false;
		}
...
// 			<< " jewStrMod=" << PVPjewStrMod << " jewClQaMod=" << PVPjewClQaMod << " jewClQaMod=" << PVPjewClQbMod << LogStream::eos;
		...
		if (dataEntry)
		{
			m_EnhanceNum = dataEntry->enhanceAttribute;
			m_EnhancePVPNum = dataEntry->enhanceAttributePVP;
		}
		...
		if (attrData->independAtk > 0)
		{
			...
			strengRate = EquipIndpendatkStrengModDataEntryMgr::Instance()->GetStrengRate(EquipIndpendatkStrengModDataEntryMgr::PVPID, GetStrengthen());
			colorARate = EquipIndpendatkStrengModDataEntryMgr::Instance()->GetColorARate(EquipIndpendatkStrengModDataEntryMgr::PVPID, GetQuality());
			colorBRate = EquipIndpendatkStrengModDataEntryMgr::Instance()->GetColorBRate(EquipIndpendatkStrengModDataEntryMgr::PVPID, GetQuality());
			partRate = EquipIndpendatkStrengModDataEntryMgr::Instance()->GetPartRate(EquipIndpendatkStrengModDataEntryMgr::PVPID);
			m_IndependAtkPvpStreng = ((GetNeedLevel() + (colorARate / 100)) / 8) * strengRate / 100 * colorBRate / 100 * 1.1  * partRate / 100 * 1000;
		}
		else
		{
			m_IndependAtkStreng = 0;
...
// 		" PVEDisMagDef=" << m_DisMagDef << " PVPDisMagDef=" << m_PvpDisMagDef << LogStream::eos;
	...
	if (IsAssistEquip(GetSubType()))
	{
		...
		float fQlRate = (float)GetQualityLv() / 100.00f;
		UInt8 bPerfect = 0;
		if (GetQualityLv() >= 100)
		{
			bPerfect = 1;
...
bool Item::CanReduceStrengthenOrEnhanceLv()
{
	if (GetValue(IKVKT_WBP_SPECIAL) != WBS_FORVER)
	{
		return true;
	}
	
	if (Avalon::Random::SelectInHundred(GetValue(IKVKT_WBP_SPECIAL_DETAIL)/10))
	{
		return false;
	}
...
void Item::ChgEquipType(EquipType equipType)
{
...
void Item::SetSingleQualityProp(EquipQLAttrType type, UInt8 ql, EquipAttrDataEntry* attrData, EquipQLValueData* qlValueData)
{
	...
	case EQAT_INDEPENDATK:{ SET_FOURD_PROP(attrData->independAtk, indpendAtkRatio, perIndpendAtkRatio, SetIndependAtk) } break;
	default:	break;
	}
}
...
bool Item::SetFashionDefaultAttr()
{
	...
	FashionAttrData* data = FashionAttrDataMgr::Instance()->FindEntry(GetEqPropId());
	if (!data)
	{
...
void Item::SetFashionAttrInherentId()
{
	if (GetType() != ITEM_FASHION)
	{
		return;
	...
	m_AttrDataInherentId = data->defaultAttr1;
}
...
Int16 Item::GetTownSpeedRate()
{
	UInt16	movespeedAddition = 0;
	if (GetAttrDataEntry())
	{
		movespeedAddition += GetAttrDataEntry()->TownSpeedRate;
	}
	if (GetAttrDataEntryInherent())
	{
		movespeedAddition += GetAttrDataEntryInherent()->TownSpeedRate;
	}
	return movespeedAddition;
}
...
float Item::GetBasePropScore()
{
	...
	float score = \
		...
		GetScoreByPropType(ISP_INDEPENDATK));
...
ItemScoreProp Item::TransRandPropType(ItemRandProp rp)
{
	...
	case IRP_INDEPEND_ATK:	    return ISP_INDEPENDATK;
	default:					break;
	}
...
EquipPartType Item::GetPartType()
{
	...
	else if (IsAssistEquip(GetSubType()))
	{
		return EPT_ASSIST;
	}
	else if (IsMagicStoneEquip(GetSubType()))
	{
		return EPT_MAGICSTONE;
	}
	else
	{
		return EPT_NONE;
...
bool Item::SetAttribute(bool isDynamic)
{
	if (!GetDataEntry())
	{
		return false;
	...
	if (GetType() == ITEM_EQUIP)
	{
		EquipAttrDataEntry* attrData = nullptr;
		//设置属性data
		if (!isDynamic)
		{
			attrData = EquipAttrDataMgr::Instance()->FindEntry(GetDataEntry()->attrId);
		}
		else
		{
			attrData = GenVirtualAttributeData();
		}
		if (!attrData)
		{
	...
	else if (GetType() == ITEM_FASHION)
	{
		...
		//设置固有属性
		if (GetFashionAttrInherentId() == 0)
		{
			SetFashionAttrInherentId();
			EquipAttrDataEntry* attrData1 = EquipAttrDataMgr::Instance()->FindEntry(GetFashionAttrInherentId());
			if (attrData1)
			{
				SetAttrDataEntryInherent(attrData1);
			}
		}
		return true;
	}
...
EquipAttrDataEntry* Item::GenVirtualAttributeData()
{
// 	if (GetValue(IKVKT_WBP_BUILD_WEAPON) == 0)
// 	{
// 		return nullptr;
// 	}
	EquipAttrDataEntry* dataEntry = GetAttrDataEntry();
	if (dataEntry == nullptr)
	{
		dataEntry = new EquipAttrDataEntry;
	}

	if (dataEntry == nullptr)
	{
		ErrorLogStream << "gen virtual attribute data faild! " << this->GetID() << LogStream::eos;
		return nullptr;
	}

	for (UInt32 i = IKVKT_NONE; i < IKVKT_WBP_MAX; ++i)
	{
		if (i == IKVKT_WBP_STR)
		{
			dataEntry->strenth = GetValue(i);
		}
		else if (i == IKVKT_WBP_STA)
		{
			dataEntry->stamina = GetValue(i);
		}
		else if (i == IKVKT_WBP_INT)
		{
			dataEntry->intellect = GetValue(i);
		}
		else if (i == IKVKT_WBP_SPR)
		{
			dataEntry->spirit = GetValue(i);
		}
		else if (i == IKVKT_WBP_HPMAX)
		{
			dataEntry->HPMax = GetValue(i);
		}
		else if (i == IKVKT_WBP_MPMAX)
		{
			dataEntry->MPMax = GetValue(i);
		}
		else if (i == IKVKT_WBP_HPREC)
		{
			dataEntry->HPRecover = GetValue(i);
		}
		else if (i == IKVKT_WBP_MPREC)
		{
			dataEntry->MPRecover = GetValue(i);
		}
		else if (i == IKVKT_WBP_HIT)
		{
			dataEntry->HitRate = GetValue(i);
		}
		else if (i == IKVKT_WBP_DEX)
		{
			dataEntry->AvoidRate = GetValue(i);
		}
		else if (i == IKVKT_WBP_PHYCRT)
		{
			dataEntry->PhysicCrit = GetValue(i);
		}
		else if (i == IKVKT_WBP_MGCCRT)
		{
			dataEntry->MagicCrit = GetValue(i);
		}
		else if (i == IKVKT_WBP_ATKSPD)
		{
			dataEntry->AtkSpeedRate = GetValue(i);
		}
		else if (i == IKVKT_WBP_RDYSPD)
		{
			dataEntry->FireSpeedRate = GetValue(i);
		}
		else if (i == IKVKT_WBP_MOVSPD)
		{
			dataEntry->BattleSpeedRate = GetValue(i);
		}
		else if (i == IKVKT_WBP_JUMP)
		{
			//dataEntry->intellect = GetValue(i);
		}
		else if (i == IKVKT_WBP_HITREC)
		{
			dataEntry->Spasticity = GetValue(i);
		}
		else if (i == IKVKT_WBP_PHYATK)
		{
			dataEntry->atk = GetValue(i);
		}
		else if (i == IKVKT_WBP_MAGATK)
		{
			dataEntry->magicAtk = GetValue(i);
		}
		else if (i == IKVKT_WBP_PHYDEF)
		{
			dataEntry->def = GetValue(i);
		}
		else if (i == IKVKT_WBP_MAGDEF)
		{
			dataEntry->magicDef = GetValue(i);
		}
		else if (i == IKVKT_WBP_LIGHT)
		{
			dataEntry->AtkPropEx[EEP_LIGHT] = GetValue(i);
		}
		else if (i == IKVKT_WBP_FIRE)
		{
			dataEntry->AtkPropEx[EEP_FIRE] = GetValue(i);
		}
		else if (i == IKVKT_WBP_ICE)
		{
			dataEntry->AtkPropEx[EEP_ICE] = GetValue(i);
		}
		else if (i == IKVKT_WBP_DARK)
		{
			dataEntry->AtkPropEx[EEP_DARK] = GetValue(i);
		}
		else if (i == IKVKT_WBP_LIGHT_ATK)
		{
			dataEntry->StrPropEx[EEP_LIGHT] = GetValue(i);
		}
		else if (i == IKVKT_WBP_FIRE_ATK)
		{
			dataEntry->StrPropEx[EEP_FIRE] = GetValue(i);
		}
		else if (i == IKVKT_WBP_ICE_ATK)
		{
			dataEntry->StrPropEx[EEP_ICE] = GetValue(i);
		}
		else if (i == IKVKT_WBP_DARK_ATK)
		{
			dataEntry->StrPropEx[EEP_DARK] = GetValue(i);
		}
		else if (i == IKVKT_WBP_LIGHT_DEF)
		{
			dataEntry->DefPropEx[EEP_LIGHT] = GetValue(i);
		}
		else if (i == IKVKT_WBP_FIRE_DEF)
		{
			dataEntry->DefPropEx[EEP_FIRE] = GetValue(i);
		}
		else if (i == IKVKT_WBP_ICE_DEF)
		{
			dataEntry->DefPropEx[EEP_ICE] = GetValue(i);
		}
		else if (i == IKVKT_WBP_DARK_DEF)
		{
			dataEntry->DefPropEx[EEP_DARK] = GetValue(i);
		}
		else if (i == IKVKT_WBP_GDKX)
		{
			dataEntry->AbnormalResists[EAR_FLASH] = GetValue(i);
		}
		else if (i == IKVKT_WBP_CXKX)
		{
			dataEntry->AbnormalResists[EAR_BLEEDING] = GetValue(i);
		}
		else if (i == IKVKT_WBP_ZSKX)
		{
			dataEntry->AbnormalResists[EAR_BURN] = GetValue(i);
		}
			else if (i == IKVKT_WBP_ZDKX)
		{
			dataEntry->AbnormalResists[EAR_POISON] = GetValue(i);
		}
		else if (i == IKVKT_WBP_SMKX)
		{
			dataEntry->AbnormalResists[EAR_BLIND] = GetValue(i);
		}
		else if (i == IKVKT_WBP_XYKX)
		{
			dataEntry->AbnormalResists[EAR_STUN] = GetValue(i);
		}
		else if (i == IKVKT_WBP_SHKX)
		{
			dataEntry->AbnormalResists[EAR_STONE] = GetValue(i);
		}
		else if (i == IKVKT_WBP_BDKX)
		{
			dataEntry->AbnormalResists[EAR_FROZEN] = GetValue(i);
		}
		else if (i == IKVKT_WBP_SLKX)
		{
			dataEntry->AbnormalResists[EAR_SLEEP] = GetValue(i);
		}
		else if (i == IKVKT_WBP_HLKX)
		{
			dataEntry->AbnormalResists[EAR_CONFUNSE] = GetValue(i);
		}
		else if (i == IKVKT_WBP_SFKX)
		{
			dataEntry->AbnormalResists[EAR_STRAIN] = GetValue(i);
		}
		else if (i == IKVKT_WBP_JSKX)
		{
			dataEntry->AbnormalResists[EAR_SPEED_DOWN] = GetValue(i);
		}
		else if (i == IKVKT_WBP_ZZKX)
		{
			dataEntry->AbnormalResists[EAR_CURSE] = GetValue(i);
		}
		else if (i == IKVKT_WBP_INDEPEND_ATK)
		{
			dataEntry->independAtk = GetValue(i);
		}
		else if (i == IKVKT_WBP_PhySkillMp)
		{
			dataEntry->phySkillMp = GetValue(i);
		}
		else if (i == IKVKT_WBP_PhySkillCd)
		{
			dataEntry->phySkillCd = GetValue(i);
		}
		else if (i == IKVKT_WBP_MagSkillMp)
		{
			dataEntry->magSkillMp = GetValue(i);
		}
		else if (i == IKVKT_WBP_MagSkillCd)
		{
			dataEntry->magSkillCd = GetValue(i);
		}
		else if (i == IKVKT_WBP_QSKX)
		{
			//dataEntry->magSkillCd = GetValue(i);
		}
		else if (i >= IKVKT_WBP_BUFF_BEGIN && i <= IKVKT_WBP_BUFF_END)
		{
			if (GetValue(i) > 0)
			{
				dataEntry->attchBuffInfoIds.push_back(GetValue(i));
			}
		}
	}

	dataEntry->skillScore = GetValue(IKVKT_WBP_ADDITION_SCORE);
	return dataEntry;
}

void Item::ClearAttribute()
{
	SetPhyAtk(0);
	SetMagAtk(0);
	SetPhyDef(0);
	...
	for (size_t i = 0; i < (sizeof(m_StrPropEx) / sizeof(m_StrPropEx[0])); ++i)
	{
		m_StrPropEx[i] = 0;
	}

	for (size_t i = 0; i < (sizeof(m_DefPropEx) / sizeof(m_DefPropEx[0])); ++i)
	{
		m_DefPropEx[i] = 0;
	}
	...
	for (size_t i = 0; i < (sizeof(m_AbnormalResists) / sizeof(m_AbnormalResists[0])); ++i)
	{
		m_AbnormalResists[i] = 0;
	}
}
...
bool Item::GenDataAttrs()
{
	...
	SetIndependAtk(GetAttrDataEntry()->independAtk);
...
float Item::GetStrPropScore()
{
	...
	//计算独立攻击力强化
	float value = GetIndpentAtkStrScore();
	score += value;
	...
	DebugLogStream << "disPhy:" << m_DisPhyAtk \
		...
		<< ", indepentAtkStreng : " << m_IndependAtkStreng \
...
float Item::GetMagicPropScore()
{
	...
	UInt8 magicLev = m_AddMagic.GetMagicCardLev();
	...
	for (size_t i = 0; i < data->props.size(); ++i)
	{
		...
		UInt32 fix = 1;
		if ((sp >= ISP_STR && sp <= ISP_SPR) || sp == ISP_INDEPENDATK)
		{
			fix = 1000;
		}
		else if (sp >= ISP_ATKSPD && sp <= ISP_MISS)
		...
		float value = (float)(mp.propValue + magicLev * data->upValue) / fix;
		float rate = (float)scoreData->value / 1000;
		if (fix == 10)
		{
			UInt16 lv = GetNeedLevel() < 5 ? 5 : GetNeedLevel();
...
float Item::GetPreciousBeadPropScore()
{
	...
	for (UInt32 i = 1; i < PRECBEAD_MAXNUM; i++)
	{
		...
		else if (dataEntry->buffsPve.size() > 0)
		{
...
float Item::GetInscriptionScore()
{
	float score = 0;

	std::vector<UInt32> Ids;
	m_InscriptionHoles.GetRaceInfo(Ids);

	for (auto id : Ids)
	{
		InscriptionDataEntry* dataEntry = InscriptionDataEntryMgr::Instance()->FindEntry(id);
		if (dataEntry)
		{
			score += dataEntry->score;
		}
...
float	Item::GetIndpentAtkStrScore()
{
	float score = 0.0f;
	EquipValueSceorData* scoreData = EquipValueSceorDataMgr::Instance()->FindEntry(ISP_INDEPENDATK);
	if (!scoreData)		return 0;
	float rate = (float)scoreData->value / 1000;
	UInt32 value = this->GetIndependAtkStreng() / 1000;
	score = value * rate;
	DebugLogStream << "item id:" << GetID() << ", indpent atk streng score:" << score << LogStream::eos;
	return score;
}
...
float Item::GetScoreByPropType(ItemScoreProp type)
{
	...
	float enhanceValueStrenth = 0;
	float enhanceValueIntellect = 0;
	float enhanceValueStamina = 0;
	float enhanceValueSpirit = 0;
	if (GetEnhanceType() == ENHANCE_STRENTH)
	{
		enhanceValueStrenth = m_EnhanceNum * 1000;
	}
	else if (GetEnhanceType() == ENHANCE_INTELLECT)
	{
		enhanceValueIntellect = m_EnhanceNum * 1000;
	}
	else if (GetEnhanceType() == ENHANCE_STAMINA)
	{
		enhanceValueStamina = m_EnhanceNum * 1000;
	}
	else if (GetEnhanceType() == ENHANCE_SPIRIT)
	{
		enhanceValueSpirit = m_EnhanceNum * 1000;
	}

	switch (type)
	{
	case ISP_STR:		value = ((float)GetStrenth() + enhanceValueStrenth) / 1000;	break;
	case ISP_INT:		value = ((float)GetIntellect() + enhanceValueIntellect) / 1000;	break;
	case ISP_STAM:		value = ((float)GetStamina() + enhanceValueStamina) / 1000;	break;
	case ISP_SPR:		value = ((float)GetSpirit() + enhanceValueSpirit) / 1000;	break;
	...
	case ISP_INDEPENDATK: value = (float)GetIndependAtk() / 1000; break;
	default:		return 0;
...
float Item::GetPartRate()
{
	...
	case TT_ITEM_SPEAR:			type = ISP_SPEAR; break;
	case TT_ITEM_STICK:			type = ISP_STICK; break;
	...
	if (data)
	{
		for (size_t i = 0; i < data->props.size(); ++i)
		{
			...
			else if (sp >= ISP_LIGHT_ATK && sp <= ISP_DARK_ATK)
			{
				...
				StrPropEx[idx] = StrPropEx[idx] + mp.propValue + magicLev * data->upValue;
			}
			else if (sp >= ISP_LIGHT_DEF && sp <= ISP_DARK_DEF)
			{
				...
				DefPropEx[idx] = DefPropEx[idx] + mp.propValue + magicLev * data->upValue;
			}
			else if (sp >= ISP_GDKX && sp <= ISP_ZZKX)
			{
				...
				AbnormalResists[idx] = AbnormalResists[idx] + mp.propValue + magicLev * data->upValue;
			}
			else {
			}
		}
...
bool Item::SetMagicNew(Item* card)
{
	if (!card || !card->GetDataEntry())
	{
		return false;
	}
	...
	m_AddMagic.SetMagciCard(cardData->id);
	m_AddMagic.SetMagicCardLev(card->GetStrengthen());
	return true;
}

bool Item::SetMagicIdAndLv(UInt32 cardId, UInt8 lv)
{
	MagicCardData* cardData = MagicCardDataMgr::Instance()->FindEntry(cardId);
	if (!cardData)
	{
		return false;
	}

	m_AddMagic.SetMagciCard(cardId);
	m_AddMagic.SetMagicCardLev(lv);
	return true;
}
...
void Item::ClearMagic()
{
	m_AddMagic.SetMagciCard(0);
	m_AddMagic.SetMagicCardLev(0);
}
...
void Item::GenSecondPreciousBeadHoles()
{
	if (GetValue(IKVKT_WBP_SPECIAL) == WBS_DOUBLE)
	{
		this->m_PreciousBeadHoles.AddHole(2, PBMHT_COMMON);
	}
}
...
void Item::GenPreciousBeadHoleLoaded()
{
	if ((this->GetType() == ITEM_EQUIP || this->GetType() == ITEM_TITLE) && this->m_PreciousBeadHoles.HolesNum() == 0
		&& (this->GetDataEntry()->PreciousBeadHole1.size()>0 || this->GetDataEntry()->PreciousBeadHole2.size()>0 || this->GetValue(IKVKT_WBP_SPECIAL) == WBS_DOUBLE))
	{
		this->GenPreciousBeadHoles();
		GenSecondPreciousBeadHoles();
	}
}
...
UInt32 Item::GetSecondPreciosBeadId()
{
	return  this->m_PreciousBeadHoles.GetPreciousBeadId(2);
}

void Item::ClearPrecBead()
{
	this->m_PreciousBeadHoles.ClearPrecbead();
}
...
bool Item::CheckDueTimeForAuction(Player* player)
{
	if (m_uDeadLine == 0)
	{
		return true;
	...
	UInt32 hours = SystemValueDataEntryMgr::Instance()->GetSystemValue(SVT_AUCTION_ONSALE_DUETIME_REMAIN);
	if (m_uDeadLine > 0 && m_uDeadLine < ((UInt32)CURRENT_TIME.Sec() + hours * HOUR_TO_SEC))
	{
...
bool Item::CheckAuctionTransNum()
{
	ItemDataEntry* dataEntry = GetDataEntry();
	if (!dataEntry)
	{
	...
	if (dataEntry->auctionTransNum == 0)
	{
		return true;
	}

	return dataEntry->auctionTransNum > m_AuctionTransNum;
}

void Item::SetAuctionTransNum(UInt32 auctionTransNum)
{
	this->m_AuctionTransNum = auctionTransNum;
}

UInt32  Item::GetAuctionTransNum()
{
	return m_AuctionTransNum;
}

UInt32 Item::GetFourDimenAddition()
{
	ItemDataEntry* dataEntry = GetDataEntry();
...
UInt32 Item::GetFourDimenAdditionPvp()
{
	ItemDataEntry* dataEntry = GetDataEntry();
	if (!dataEntry || dataEntry->subType != ST_ASSIST_EQUIP)
	{
	...
	return AssistEqStrengFouerDimAddDataEntryMgr::Instance()->GetPvpAddValue(GetStrengthen(), dataEntry->needLevel, GetQuality(), GetQualityColor2());
}

bool Item::CheckProhibitOp(UInt8 op)
{
	return GetDataEntry()->prohibitOps.test(op);
}
...
void Item::CalcItemScore(Player* player)
{
	...
	itemScore += GetInscriptionScore();

	SetItemScore(itemScore);
}

UInt32 Item::GetPVPAssistStrength()
{
	return GetStrenth() - GetFourDimenAddition() + GetFourDimenAdditionPvp();
}

UInt32 Item::GetPVPAssistStamina()
{
	return GetStamina() - GetFourDimenAddition() + GetFourDimenAdditionPvp();
}

UInt32 Item::GetPVPAssistIntellect()
{
	return GetIntellect() - GetFourDimenAddition() + GetFourDimenAdditionPvp();
}

UInt32 Item::GetPVPAssistSpirit()
{
	return GetSpirit() - GetFourDimenAddition() + GetFourDimenAdditionPvp();
}

UInt32 Item::GetPVPIndependAtk()
{
	UInt32 ratio = SSApplication::Instance()->GetIndpentPvpRatio();
	return (UInt64)GetIndependAtk() * ratio / 1000;
}

UInt32 Item::GetPVPIndependAtkStreng()
{
	return m_IndependAtkPvpStreng;
}

bool Item::IsCanPushScheme()
{
	if (GetType() != ITEM_EQUIP && GetType() != ITEM_TITLE)
	{
		return false;
	}
	if (GetSubType() == ST_EQUIP_WEAPON)
...
void  Item::EquipInscriptionModifyByEqChage()
{
	if (GetInscriptionHoleProperty().HolesNum() == 0)
	{
		return;
	}
	UInt32 key = EquipInscriptionHoleDataEntry::MakeKey(this->GetDataEntry()->color, this->GetDataEntry()->color2, this->GetDataEntry()->subType);
	EquipInscriptionHoleDataEntry* dataEntry = EquipInscriptionHoleDataEntryMgr::Instance()->FindEntry(key);
	if (!dataEntry)
	{
		return;
	}
	for (auto h : dataEntry->holeVec)
	{
		InscriptionMountHole* hole = this->GetInscriptionHoleProperty().FindHole(h.index);
		if (!hole)
		{
			continue;
		}
		hole->type = h.type;
		this->GetInscriptionHoleProperty().SetDirty();
	}
}

bool Item::SetValue(UInt32 key, Int32 value)
{
	return m_KeyValues.SetValue(key, value);
}

Int32	Item::GetValue(UInt32 key)
{
	return m_KeyValues.GetValue(key);
}

void RewardGroup::AddReward(UInt32 id, UInt32 num, UInt8 qualityLv, UInt8 strenth,
	UInt32 auctionCoolTimeStamp, EquipType equipType, UInt32 auctionTransTimes)
{
	if (num > 0)
	{
		m_Rewards.push_back(ItemReward(id, num, qualityLv, strenth, auctionCoolTimeStamp, equipType, auctionTransTimes));
	}
}

void RewardGroup::AddRewardScript(UInt32 id, UInt32 num, UInt8 qualityLv, UInt8 strenth)
{
	if (num > 0)
	{
		m_Rewards.push_back(ItemReward(id, num, qualityLv, strenth, 0, EQUIP_NORMAL, 0));
	}
}
