...
public:
	...
	*@brief 爬塔任务类型
	*/
	virtual UInt32 LostDungeonTaskType() { return 0; }
...
*@brief 爬塔任务
*/
class LostDungeonTask : public BaseTask
{
	AVALON_DEFINE_OBJECTPOOL(LostDungeonTask, Avalon::NullMutex)

public:
	LostDungeonTask(Player* owner, UInt32 id);
	LostDungeonTask(Player* owner, UInt32 id, UInt64 guid, TaskStatus status);
	...
	*@brief 获取任务子类型
	*/
	virtual TaskSubType GetSubType() const;
	...
	*@brief 获取任务限时
	*/
	virtual UInt32 GetLimitTime() const;

	/**
	*@brief 获取任务放弃限时
	*/
	virtual UInt32 GetAbandonLimitTime() const;
	...
	*@brief 获取任务接取次数
	*/
	virtual UInt32 GetMaxSubmitCount() const;

	/**
	*@brief 是否需要公告
	*/
	virtual bool IsAnnouncement() const;

	/**
	*@brief 爬塔任务类型
	*/
	virtual UInt32 LostDungeonTaskType();
...
public:
	...
	void SetDataEntry(LostDungeonMissionDataEntry* entry) { m_Entry = entry; }
	const LostDungeonMissionDataEntry* GetDataEntry() { return m_Entry; }

private:

	LostDungeonMissionDataEntry* m_Entry;
};
