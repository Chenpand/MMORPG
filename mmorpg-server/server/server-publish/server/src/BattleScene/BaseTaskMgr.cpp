...
void BaseTaskMgr::OnPutFashion(ItemSubType subType, UInt8 color)
{
	class OnPutFashionVisitor : public BaseTaskVisitor
	{
	public:
		OnPutFashionVisitor(ItemSubType subType, UInt8 color) : m_ItemSubType(subType), m_Color(color){}
		...
		bool operator()(BaseTask* task)
		{
			...
			{
				TaskScript* script = ScriptMgr::Instance()->FindTaskScript(task->GetDataId());
				if (script != NULL && script->CheckPutFashionEvent(m_ItemSubType, m_Color))
				{
	...
	OnPutFashionVisitor visitor(subType, color);
	VisitTasks(visitor);
