...
void HandleProtocol(CLProtocol::WorldRegisterPlayerRet& protocol)
{
	...
	DebugLogStream << "player(" << player->GetAccID() << ", " << player->GetID() << ") login finish." << LogStream::eos;
}
...
void HandleProtocol(CLProtocol::SceneGameReconnectRet& protocol)
{
	...
	else if (player->GetScene()->GetType() == SCENE_TYPE_PK || player->GetScene()->GetType() == SCENE_TYPE_ACTIVITY || (player->GetScene()->GetDataEntry() && player->GetScene()->GetDataEntry()->subType == SCENE_SUB_ROOM))
	{
		...
		if (lastScene && lastScene->GetDataEntry() && lastScene->GetDataEntry()->subType == SCENE_SUB_ROOM)
		{
...
void HandleProtocol(Player* player, CLProtocol::SceneSyncWallowFactor& protocol)
{
