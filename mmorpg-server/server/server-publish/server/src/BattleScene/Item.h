...
public:
	...
	*/
	 ...
	 *@param num 道具数量
	 */
	UInt16 CheckMerging(UInt16 num, UInt32 auctionCoolTime);
	UInt16 Merge(const char* reason, Player* player, UInt16 num, UInt32 auctionCoolTimeStamp);
	...
	*/
	...
	inline void SetStrengthenValue (UInt8 str)	{ (GetType() == ITEM_EQUIP) ? m_Strengthen = str : m_Strengthen = 0; }
...
public:
	 ...
	 */
	void AddReward(UInt32 id, UInt32 num, UInt8 qualityLv = 0, UInt8 strenth = 0, UInt32 auctionCoolTimeStamp = 0);
