 ...
 *@brief buff管理器
 */
class BuffMgr : public CLComplexProperty
{
	//最大buff数
...
public:	//事件
	...
	bool GetDungeonBufferAddition(DungeonAddition& addition);
	...
	*@brief 添加运营活动buff
	*/
	void AddOpActBuff(UInt32 buffid, UInt32 duration);
	...
	*@brief 根据buff类型获取加成
	*/
	Int32 GetBuffAddition(BuffType buffType);
...
private:
	...
	*@brief 增加buff效果
	*/
	void _AddBuffEffect(UInt32 buffId);

	/**
	*@brief 移除buff效果
	*/
	void _RemoveBuffEffect(UInt32 buffId);
