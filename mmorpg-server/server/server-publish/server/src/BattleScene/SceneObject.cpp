...
bool SceneObject::ChangeScene(UInt32 sceneid, const CLPosition& pos, UInt8 dir, bool bTransZone, UInt32 targetDoorId, UInt32 dyncSceneId)
{
	...
	SceneDataEntry* dataEntry = SceneDataEntryMgr::Instance()->FindEntry(sceneid);
	if (!dataEntry)
	{
		return false;
	}
	UInt32 realSceneId = dyncSceneId > 0 ? dyncSceneId : sceneid;
	if (!IsInScene())
	{
		return false;
	}
	
	if(GetSceneID() == realSceneId)
	{
		return false;
	}
	...
	Scene* scene = SceneMgr::Instance()->FindNormalScene(realSceneId);
...
Avalon::Packet* SceneObject::MakeSyncObjectPacket(bool bEnterScene)
{
	...
	try
	{
		stream & UInt8(bEnterScene?1:0) & GetScene()->GetMapID();
		stream & GetID() & UInt8(GetType());
		Encode(stream, SST_AROUND | SST_MOVE | SST_SCENE_ENTER, false);
		stream & ObjID_t(0);
	}
