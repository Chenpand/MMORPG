...
public:
	SelectForbidTalkTimeCb(UInt32 connid, UInt32 forbidTime)
		:m_ConnId(connid), m_ForbidTime(forbidTime){}
	...
	void OnFinished()
	{
		...
		while (NextRow())
		{
			...
			UInt32 dueTime = GetData("forbid_talk_duetime");

			if (m_ForbidTime == 0)
			{
				CLUpdateCommand* updateCmd = CLRecordClient::Instance()->PrepareUpdateCommand("t_player_info", GetKey(), true);
				updateCmd->PutData("forbid_talk_duetime", 0);
				CLRecordClient::Instance()->SendCommand(updateCmd);
				continue;
			}

			Avalon::Time now = Avalon::Time::CurrentTime();
			if (dueTime > (UInt32)now.Sec())
			{
				CLUpdateCommand* updateCmd = CLRecordClient::Instance()->PrepareUpdateCommand("t_player_info", GetKey(), true);
				updateCmd->PutData("forbid_talk_duetime", dueTime + m_ForbidTime);
				CLRecordClient::Instance()->SendCommand(updateCmd);
			}
			else
			{
				CLUpdateCommand* updateCmd = CLRecordClient::Instance()->PrepareUpdateCommand("t_player_info", GetKey(), true);
				updateCmd->PutData("forbid_talk_duetime", (UInt32)now.Sec() + m_ForbidTime);
				CLRecordClient::Instance()->SendCommand(updateCmd);
			}		
		}
			...
			bool operator()(Item* item)
			{
				if (m_itemid > 0)
				{
					if (item->GetDataID() == m_itemid)
					{
						...
						obj["r"] = json::String(Avalon::StringUtil::ConvertToString<UInt32>(item->GetPreciosBeadId()));
...
void SSConsoleAgent::ForbidTalkPlayer(UInt32 connid, UInt8 keyType, UInt32 accId, ObjID_t playerId, const std::string& playerName, UInt32 forbidTime)
{
	...
	else
	{
		...
		CLRecordClient::Instance()->SendCommand(selectCmd, new SelectForbidTalkTimeCb(connid, forbidTime));
	}
}
