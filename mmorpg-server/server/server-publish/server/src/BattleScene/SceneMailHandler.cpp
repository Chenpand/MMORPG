...
void HandleProtocol(CLProtocol::SceneMailCheckPackageReq& req)
{
	...
	for(std::map<ObjID_t, std::vector<ItemReward> >::reverse_iterator iter = req.items.rbegin(); iter != req.items.rend(); ++iter)
	{
		...
		for (size_t i = 0; i < items.size(); ++i)
		{
			...
			ItemReward item(items[i].id, items[i].num, items[i].qualityLv, items[i].strenth, items[i].auctionCoolTimeStamp);
...
void MailGiveItemsFunction::operator()(Avalon::Packet* packet) const
{
	...
	for (size_t i = 0; i < items.size(); ++i)
	{
		...
		{
			group.AddReward(items[i].id, items[i].num, 0, items[i].strenth, items[i].auctionCoolTimeStamp);
		}
		else
		{
			group.AddReward(items[i].id, items[i].num, 0, items[i].strenth, items[i].auctionCoolTimeStamp);
	...
	while(id != 0)
	{
		...
		else
		{
			...
			player->SendItemUdpLog(reason.c_str(), itemId, itemDataId, itemType, itemSubType, itemName, itemQuality, 1, 0);
		}
		stream & id;
	}
}
