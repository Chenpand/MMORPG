...
void HandleProtocol(Player* player,CLProtocol::SceneChat& protocol)
{
	...
	if (protocol.channel != ChatChannelType::CHAT_CHANNEL_BATTLE
		&& protocol.channel != ChatChannelType::CHAT_CHANNEL_BAT_LOST_TEAM
		&& protocol.channel != ChatChannelType::CHAT_CHANNEL_TRIBE)
	{
		return;
	}

	if (protocol.channel == ChatChannelType::CHAT_CHANNEL_TRIBE)
	{
		Router::SendToSelfScenePlayer(player->GetSrcSceneNodeID(), player->GetID(), protocol);
		return;
	}
