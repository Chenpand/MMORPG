...
public:
	...
	void OnFailed()
	{
		ErrorLogStream << "Player(" << m_PlayerId << ")" << "Insert Counter name : " << m_CounterName.c_str() << " value(" << m_Value << ") " << LogStream::eos;
	}
...
public:
	...
	void OnFailed()
	{
		ErrorLogStream << "Player(" << m_PlayerId << ")" << "Update Counter Id : " << (UInt64)m_CounterId << " value(" << m_Value << ") " << LogStream::eos;
	}
