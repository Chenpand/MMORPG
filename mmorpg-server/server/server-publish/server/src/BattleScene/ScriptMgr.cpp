...
ScriptConfig ScriptMgr::s_scriptConfigs[] = {
	...
	{ LOST_DUNGEON_TASK_SCRIPT_PATH, "task_", NULL, true, false }
...
bool ScriptMgr::LoadScript(UInt32 type)
{
	...
	else if (SScriptType::SCENE_SCRIPT_LOST_DUNGEON_TASK == (SScriptType)type)
	{
		return LoadLostDungeonTaskScript();
	}
	else
	{
...
// 	return NULL;
	...
	{
	case SCENE_SCRIPT_TASK:
	case SCENE_SCRIPT_LOST_DUNGEON_TASK:
		return new TaskScript(id, state);
		break;
	case SCENE_SCRIPT_ITEM:
		return new ItemScript(id, state);
...
bool ScriptMgr::LoadLostDungeonTaskScript()
{
	const ScriptConfig& config = s_scriptConfigs[6];
	std::string root = CONFIG_PATH;
	std::string scriptPath = root + config.path;
	std::string scriptPrefix = config.prefix;
	SScriptType type = SScriptType::SCENE_SCRIPT_LOST_DUNGEON_TASK;

	auto& scriptMgr = GetScriptMgr(type);
	std::string postfix = ".lua";

	class TaskScriptVisiotr : public Avalon::DataEntryVisitor<LostDungeonMissionDataEntry>
	{
	public:
		TaskScriptVisiotr() {}
		~TaskScriptVisiotr() {}

		bool operator()(LostDungeonMissionDataEntry* entry)
		{
			m_Tasks.push_back(entry);
			return true;
		}

		std::vector<LostDungeonMissionDataEntry*> GetTasks() { return m_Tasks; }

	private:
		std::vector<LostDungeonMissionDataEntry*> m_Tasks;
	};

	TaskScriptVisiotr visitor;
	LostDungeonMissionDataEntryMgr::Instance()->Visit(visitor);

	std::vector<LostDungeonMissionDataEntry*> tasks = visitor.GetTasks();
	std::vector<LostDungeonMissionDataEntry*>::iterator iter = tasks.begin();
	while (iter != tasks.end())
	{
		UInt32 id = (*iter)->id;
		if (scriptMgr.find(id) != scriptMgr.end())
		...
		std::string env = scriptPrefix + Avalon::StringUtil::ConvertToString(id);
		std::string fileName = scriptPrefix + Avalon::StringUtil::ConvertToString(id) + postfix;
		m_pState->MakeEnv(env.c_str(), "SetSandBox");
		TaskScript* script = (TaskScript*)CreateScript(type, m_pState, id);
		std::string fullPath = scriptPath + fileName;
		if (!script || !script->Load(fullPath.c_str(), env.c_str()))
...
TaskScript* ScriptMgr::FindLostDungeonTaskScript(UInt32 id)
{
#ifndef _NEW_SCRIPT_MGR_
	TaskScriptMap::iterator iter = m_TaskScripts.find(id);
	if (iter != m_TaskScripts.end()) return iter->second;
	return NULL;
#else
	return GetScript<TaskScript>(SCENE_SCRIPT_LOST_DUNGEON_TASK, id);
#endif
}
...
void ScriptMgr::DefClasses()
{
	...
	m_pState->DefClass<RewardGroup>("RewardGroup")
		.Def("AddReward",&RewardGroup::AddReward);
	...
	m_pState->DefClass<Player,Creature>("Player")
		...
		.Def("StudyChiJiSkill", &Player::StudyChiJiSkill)
		;
	...
	m_pState->DefClass<TaskScript>("TaskScript")
		...
		.Def("BindLostDungeonKillRoleNumEvent", &TaskScript::BindLostDungeonKillRoleNumEvent)
		.Def("BindLostDungeonAliveTimeEvent", &TaskScript::BindLostDungeonAliveTimeEvent)
		.Def("BindLostDungeonClearEvent", &TaskScript::BindLostDungeonClearEvent)
		.Def("BindLostDungeonKillMonsterEvent", &TaskScript::BindLostDungeonKillMonsterEvent)
		.Def("BindLostDungeonGetItemEvent", &TaskScript::BindLostDungeonGetItemEvent)
		;
	...
	*/
	...
	m_pState->DefClass<BaseTask>("BaseTask")
		...
		.Def("SyncData", &BaseTask::SyncData)
		;
