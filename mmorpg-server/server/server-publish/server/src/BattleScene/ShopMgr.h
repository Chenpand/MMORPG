...
public:
	// 选择职业后刷新
	void SelectOccuRefresh();
	// 刷新吃鸡商店id
	void OnRefreshChiJiShopID(UInt32 shopId, UInt32 nextTime);
	// 初始化商店
	void InitChiJiShop(UInt32 shopId);
	// 初始化吃鸡商店数据
	void ResetChiJiData();
	// 获取当前吃鸡商店
	UInt32 GetCurChiJiShopID() { return m_curChijiShopId; }
public:
