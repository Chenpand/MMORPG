function OnUseSelf(player, item)
	...
	local reason="2012;800000110;0"
	if job == 12 then
		local giftpack = {item1 = {id=132774001,num=1}}
	...
	elseif job == 22 then
		local giftpack = {item1 = {id=132774002,num=1}}
		local group = player:GetRewardGroup()
		for k,v in pairs(giftpack) do
			group:AddReward(v.id, v.num)
		end	
	...
	elseif job == 33 then
		local giftpack = {item1 = {id=132744002,num=1}}
		local group = player:GetRewardGroup()
		for k,v in pairs(giftpack) do
			group:AddReward(v.id, v.num)
		end	
	...
	elseif job == 51 or job==21 then
		local giftpack = {item1 = {id=132754001,num=1}}
		local group = player:GetRewardGroup()
		for k,v in pairs(giftpack) do
			group:AddReward(v.id, v.num)
		end	
