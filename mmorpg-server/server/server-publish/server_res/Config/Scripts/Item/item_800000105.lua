function OnUseSelf(player, item)
	...
	if job == 12 then
		local giftpack = {item1 = {id=132274001,num=1}}
	...
	elseif job == 22 then
		local giftpack = {item1 = {id=132274002,num=1}}
		local group = player:GetRewardGroup()
		for k,v in pairs(giftpack) do
			group:AddReward(v.id, v.num)
		end	
	...
	elseif job == 33 then
		local giftpack = {item1 = {id=132244001,num=1}}
		local group = player:GetRewardGroup()
		for k,v in pairs(giftpack) do
			group:AddReward(v.id, v.num)
		end	
	...
	elseif job == 51 or job==21 then
		local giftpack = {item1 = {id=132254001,num=1}}
		local group = player:GetRewardGroup()
		for k,v in pairs(giftpack) do
			group:AddReward(v.id, v.num)
		end	
