function OnUseSelf(player, item)
	...
	local reason="2012;800000048;0"
	if job == 1 then
		local giftpack = {item1 = {id=531003004,num=1}}
	...
	elseif job == 2 then
		local giftpack = {item1 = {id=532003004,num=1}}
		local group = player:GetRewardGroup()
		for k,v in pairs(giftpack) do
			group:AddReward(v.id, v.num)
		end	
	...
	elseif job == 3 then
		local giftpack = {item1 = {id=533003004,num=1}}
		local group = player:GetRewardGroup()
		for k,v in pairs(giftpack) do
			group:AddReward(v.id, v.num)
		end	
	...
	elseif job == 5 then
		local giftpack = {item1 = {id=535003004,num=1}}
		local group = player:GetRewardGroup()
		for k,v in pairs(giftpack) do
			group:AddReward(v.id, v.num)
		end	
