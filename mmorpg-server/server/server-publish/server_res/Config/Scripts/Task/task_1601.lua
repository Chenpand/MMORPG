function Init(task)				---绑定(注册)该任务需要关注的事件
	task:BindStrengthenEquipEvent(true, 5)		--强化(是否关注成功, 强化等级)
	-- task:BindGetItemQualityEvent(0, 5)			--监听获取装备(装备id, 强化等级)
	task:BindGetItemStrengthenEvent(0, 5)			--监听获取装备(装备id, 强化等级)
end
