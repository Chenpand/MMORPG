function Init(task)				---绑定(注册)该任务需要关注的事件
    task:BindGetItemEvent(400000003)		---关注获得道具事件。
    task:BindKillNpcEvent(808, 201000)   -- 关注杀死指定怪物事件
    task:BindKillNpcEvent(808, 201001)   -- 关注杀死指定怪物事件
end
...
function OnAccepted(player, task)			---	接受任务时需要执行的操作
	local itemnum = player:GetItemNum(40000003);
...
function OnSubmitted(player, task)		---	提交任务时需要执行的操作
		...
		if(itemnum >= 1) then           -- 检查道具数量是否足够
			task:SetStatus(4);
			player:RemoveItem(task:GetRewardsReason(), 400000003, 1);        ---	删除变量值的道具
		end
end
...
function OnRmItem(player, task, itemid, num)
	...
	if(task:GetStatus() == 2 and itemnum < 1) then		-- 如果删除一定数量的道具后不满足完成任务的条件，则设置该任务为未完成状态
			task:SetStatus(1);
	end
end	
...
function OnKillNpc(player, task, npcid, npctype)        --- 任务过程中杀死NPC时需要执行的操作
    local itemnum = task:GetVar("parameter");
    if itemnum < 1 then
        task:AddTaskItem(400000003, 100);                   -- 添加任务道具
    end
end