...
function CheckTask(item1, item2, item3)
    if item1 >= 320 and item2 >= 1000 and item3 >= 300000 then
        return true;
    end
    return false;
end
...
function OnSubmitted(player, task)		---	提交任务时需要执行的操作
    ...
    if CheckTask(item1, item2, item3) then
        task:SetStatus(4);
        player:RemoveItem(task:GetRewardsReason(), 360000002, 320);        ---	删除变量值的道具
        player:RemoveItem(task:GetRewardsReason(), 300000105, 1000);        ---	删除变量值的道具
        player:RemoveGold(task:GetRewardsReason(), 300000)
    end
end
