...
{
	//perror("in enet_protocol_handle_connect0");
	...
	{
		perror("in enet_protocol_handle_connect1");
		return NULL;
	}
	...
	{
		//perror("in enet_protocol_handle_connect2");
		return NULL;
	}
	...
	{
		//perror("in enet_protocol_handle_connect3");
		return NULL;
	}
	...
	//perror("in enet_protocol_handle_connect4");
    return peer;
}
...
static int //处理ENET_PROTOCOL_COMMAND_DISCONNECT消息 . 在该处理函数中直接改变状态为ENET_PEER_STATE_ZOMBIE以及调用enet_peer_reset_queues 就造成在ENetPeer::dispatchedCommands来没来得及派发的command被丢失(直接造成了服务器在对战结束发送对战结果后再调用enet_peer_disconnect_now, client直接收到disconnect事件，造成结算结果丢失！）
...
enet_protocol_handle_acknowledge (ENetHost * host, ENetEvent * event, ENetPeer * peer, const ENetProtocol * command)
{
    ...
    case ENET_PEER_STATE_DISCONNECTING:
	   ...
	   printf("peer state disconnecting");
...
enet_protocol_check_timeouts (ENetHost * host, ENetPeer * peer, ENetEvent * event)
{
    ...
    while (currentCommand != enet_list_end (& peer -> sentReliableCommands))
    {
       ...
       {
		   printf("serviceTime(%u) earliestTimeout(%u) timeoutMaximum(%u) roundTripTimeout(%u) roundTripTimeoutLimit(%u) timeoutMinimum(%u).\n",
			   host->serviceTime, peer->earliestTimeout, peer->timeoutMaximum, outgoingCommand->roundTripTimeout, outgoingCommand->roundTripTimeoutLimit,
			   peer->timeoutMinimum);
          enet_protocol_notify_disconnect (host, peer, event);
...
#if !defined(WIN32) && !defined(_WIN32)  //modified by aprilliu
