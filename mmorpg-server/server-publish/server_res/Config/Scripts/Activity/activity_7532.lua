...
function Init(task)				---绑定(注册)该任务需要关注的事件
    task:BindDayChange();
    task:BindWearFashionEquip();
end

function OnAccepted(player, task)			---	接受任务时需要执行的操作
    task:SetStatusNoSync(TASK_UNFINISH); 	

    local num = player:GetWearFashionNum();
...
function OnWearFashionEquip(player, task, num)
...
function OnDayChange(player, task, preTask)		---	跨天
	...
	--真正接受任务
	if day >= 4 then
		OnWearFashionEquip(player, task, player:GetWearFashionNum())
	end
