function OnUseSelf(player, item)
	...
	local reason="2012;800000109;0"
	if job == 12 then
		local giftpack = {item1 = {id=133774003,num=1}}
	...
	elseif job == 22 then
		local giftpack = {item1 = {id=133774004,num=1}}
		local group = player:GetRewardGroup()
		for k,v in pairs(giftpack) do
			group:AddReward(v.id, v.num)
		end	
	...
	elseif job == 33 then
		local giftpack = {item1 = {id=133744001,num=1}}
		local group = player:GetRewardGroup()
		for k,v in pairs(giftpack) do
			group:AddReward(v.id, v.num)
		end	
	...
	elseif job == 51 or job==21 then
		local giftpack = {item1 = {id=133754001,num=1}}
		local group = player:GetRewardGroup()
		for k,v in pairs(giftpack) do
			group:AddReward(v.id, v.num)
		end	
