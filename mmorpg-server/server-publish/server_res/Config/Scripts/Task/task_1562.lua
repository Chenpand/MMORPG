function Init(task)				---绑定(注册)该任务需要关注的事件
    task:BindGetItemEvent(300000105)		---关注获得道具事件。
end
...
function OnAccepted(player, task)			---	接受任务时需要执行的操作
	local itemnum = player:GetItemNum(300000105);
...
function OnSubmitted(player, task)		---	提交任务时需要执行的操作
		...
		if(itemnum >= 500) then           -- 检查道具数量是否足够
			task:SetStatus(4);
			player:RemoveItem(task:GetRewardsReason(), 300000105, 500);        ---	删除变量值的道具
		end
end
...
function OnRmItem(player, task, itemid, num)
	...
	if(task:GetStatus() == 2 and itemnum < 500) then		-- 如果删除一定数量的道具后不满足完成任务的条件，则设置该任务为未完成状态
			task:SetStatus(1);
	end
end	
