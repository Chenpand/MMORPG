function Init(task)				---绑定(注册)该任务需要关注的事件
    task:BindGetItemEvent(330000107)		---关注获得道具事件。 炎骑士之魂
...
function OnSubmitted(player, task)		---	提交任务时需要执行的操作
	...
	if item1 >= 50 and item2 >= 13 then
		task:SetStatus(4);
        player:RemoveItem(task:GetRewardsReason(), 330000107, 50);        ---	删除变量值的道具
		player:RemoveItem(task:GetRewardsReason(), 360000001, 13);        ---	删除变量值的道具
...
function OnGetItem(player, task, itemid, num)		---	任务过程中获得道具时需要执行的操作
	...
	if item1 >= 50 and item2 >= 13 then						---	判断结束条件
		task:SetStatus(2)
	end
end
...
function OnRmItem(player, task, itemid, num)
	...
	if(task:GetStatus() == 2 and (item1 < 50 or item2 < 13)) then		-- 如果删除一定数量的道具后不满足完成任务的条件，则设置该任务为未完成状态
			task:SetStatus(1);
	end
end	
