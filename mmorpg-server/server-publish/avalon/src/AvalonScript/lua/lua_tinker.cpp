...
void lua_tinker::make_env(lua_State *L, const char* env, const char* sendbox_func)
{
	if(L == NULL || env == NULL) return;

	lua_pushstring(L, env);
	table envtable(L);

	if(sendbox_func != NULL)
	{
		call<void>(L, NULL, sendbox_func, envtable);
	}
...
/* excution                                                                  */ 
/*---------------------------------------------------------------------------*/ 
bool lua_tinker::dofile(lua_State *L, const char *filename,  const char* env)
{
...
void lua_tinker::print_error(lua_State *L, const char* fmt, ...)
{
	...
	va_list args;
	va_start(args, fmt);
	avalon_vsnprintf(text, 4096, fmt, args);
	va_end(args);
