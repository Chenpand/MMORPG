/********************************************************************
	...
	filename: 	e:\51.COM\Source\Vss\Client\Lobby\CommonLib\Crypt.cpp
	author:		veazhang
...
//	xtea((Int32*)pbyOutCurosr, (Int32*)arrbyFirst8Bytes, (Int32*)arrbyKey, DECRYPT_ROUNDS);
	...
	while((pbyInCursor - pbyInBuffer) < (int32_t)(nInBufferLength - 1))
	{
					  ...
					  char   arrbyKey[16] )
{
	...
	for(size_t i=0; i < (size_t)(7/*ENCRYPT_BLOCK_LENGTH_IN_BYTE - 1*/ - nPadLength); ++i)
	{
	...
	while(pbyInCursor - pbyInBuffer < (int32_t)(nInBufferLength - 8))
	{
...
size_t CXTEA::EncryptRoundN(char* pbyInBuffer, size_t nInBufferLength, char* pbyOutBuffer, size_t nOutBufferLength, 
				char arrbyKey[16], int32_t n)
{
	if(pbyInBuffer == NULL || nInBufferLength <= 0)
	{
		return 0;
	...
	// 第一段Buffer，不需要异或操作
	xtea((int32_t*)arrbyFirst8Bytes, (int32_t*)pbyOutCurosr, (int32_t*)arrbyKey, n);
	//	xtea((Int32*)pbyOutCurosr, (Int32*)arrbyFirst8Bytes, (Int32*)arrbyKey, DECRYPT_ROUNDS);
	pbyOutCurosr += ENCRYPT_BLOCK_LENGTH_IN_BYTE;
	...
	while((pbyInCursor - pbyInBuffer) < (int32_t)(nInBufferLength - 1))
	{
		...
		xtea((int32_t*)arrbySrcBuffer, (int32_t*)pbyOutCurosr, (int32_t*)arrbyKey, n);
		// 和上一块明文异或
		for(int i=0; i<ENCRYPT_BLOCK_LENGTH_IN_BYTE; ++i)
		{
			pbyOutCurosr[i] ^= pbyLast8BytesPlainData[i];
	...
	*/
	...
	xtea((int32_t*)arrbyLast8Bytes, (int32_t*)pbyOutCurosr, (int32_t*)arrbyKey, n);
	// 和上一块明文异或
	for(int i=0; i<ENCRYPT_BLOCK_LENGTH_IN_BYTE; ++i)
	{
		pbyOutCurosr[i] ^= pbyLast8BytesPlainData[i];
...
size_t CXTEA::DecryptRoundN(char* pbyInBuffer, size_t nInBufferLength, char* pbyOutBuffer, size_t nOutBufferLength, 
				char arrbyKey[16], int32_t n)
{
	if(pbyInBuffer == NULL || nInBufferLength <= 0)
	{
	...
	// 先解出最前面包含Pad的ENCRYPT_BLOCK_LENGTH_IN_BYTE个Byte
	char arrbyFirst8Bytes[ENCRYPT_BLOCK_LENGTH_IN_BYTE] = {0};
	xtea((int32_t*)pbyInCursor, (int32_t*)arrbyFirst8Bytes, (int32_t*)arrbyKey, n);
	pbyInCursor += ENCRYPT_BLOCK_LENGTH_IN_BYTE;
	// Pad长度只是用了第一个字节的最低3bit，高5bit是随机数
	int nPadLength = arrbyFirst8Bytes[0] & 0x07;
	...
	while(pbyInCursor - pbyInBuffer < (int32_t)(nInBufferLength - 8))
	{
		...
		xtea((int32_t*)arrbySrcBuffer, (int32_t*)pbyOutCursor, (int32_t*)arrbyKey, n);
		// 和上一个8char密文异或
		for(int i=0; i<8; ++i)
		{
			pbyOutCursor[i] ^= pbyLast8BytesCryptedData[i];
	...
	*/
	...
	xtea((int32_t*)arrbySrcBuffer, (int32_t*)arrbyLast8Bytes, (int32_t*)arrbyKey, n);
	// 和上一个8Byte密文异或
	for(int i=0; i<ENCRYPT_BLOCK_LENGTH_IN_BYTE; ++i)
	{
		arrbyLast8Bytes[i] ^= pbyLast8BytesCryptedData[i];
